...
create index if not exists idx_catalogo_ml_snapshot_produto_operational
  on public.catalogo_ml_snapshot (produto_id, status, catalog_listing, synced_at desc)
...
-- Repara somente produtos cujo anúncio principal ficou encerrado, embora exista
-- outro anúncio ativo observado para o mesmo produto.
with stale_products as (
  select produto.id as produto_id
  from public.produtos produto
  join public.catalogo_ml_snapshot current_snapshot
    on current_snapshot.ml_item_id = produto.ml_item_id
  where lower(coalesce(current_snapshot.status, '')) not in ('active', 'paused')
),
active_choice as (
  select distinct on (snapshot.produto_id)
    snapshot.*
  from public.catalogo_ml_snapshot snapshot
  join stale_products stale on stale.produto_id = snapshot.produto_id
  where lower(coalesce(snapshot.status, '')) = 'active'
  order by
    snapshot.produto_id,
    snapshot.catalog_listing asc,
    snapshot.synced_at desc,
    snapshot.ml_item_id
)
...
select
  ...
  snapshot.produto_id,
  produto.sku,
  coalesce(nullif(snapshot.title, ''), produto.nome),
  coalesce(snapshot.price, 0),
  case when lower(snapshot.status) = 'active' then 'ativo'::public.ml_status else 'pausado'::public.ml_status end,
  snapshot.catalog_listing,
  snapshot.thumbnail,
  snapshot.permalink,
  now()
from public.catalogo_ml_snapshot snapshot
join public.produtos produto on produto.id = snapshot.produto_id
join active_choice chosen on chosen.ml_item_id = snapshot.ml_item_id
where not exists (
  select 1
  from public.anuncios_ml anuncio
  where anuncio.ml_item_id = snapshot.ml_item_id
)
...
set
  ml_item_id = chosen.ml_item_id,
  ml_status = 'ativo'::public.ml_status,
  updated_at = now()
from active_choice chosen
where chosen.produto_id = produto.id
  and produto.ml_item_id is distinct from chosen.ml_item_id;
