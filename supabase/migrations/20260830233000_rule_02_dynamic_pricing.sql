...
alter table public.configuracoes
  add column if not exists simples_inicio_atividade date not null default date '2026-03-23',
  add column if not exists simples_aliquota_confirmada numeric(7,6),
  add column if not exists simples_aliquota_confirmada_em date;

do $rule02$
begin
  if not exists (
    select 1 from pg_catalog.pg_constraint
    where conname = 'configuracoes_simples_aliquota_confirmada_check'
      and conrelid = 'public.configuracoes'::regclass
  ) then
    alter table public.configuracoes
      add constraint configuracoes_simples_aliquota_confirmada_check
      check (simples_aliquota_confirmada is null or (simples_aliquota_confirmada >= 0.04 and simples_aliquota_confirmada < 1));
  end if;
end;
$rule02$;

create or replace function public.get_pricing_monthly_revenue(
  p_period_start date,
  p_period_end date
) returns table(revenue_month date, gross_revenue numeric)
language sql
security definer
stable
...
as $rule02$
  select
    date_trunc('month', pedido.data_venda at time zone 'America/Sao_Paulo')::date as revenue_month,
    coalesce(sum(pedido.operational_total), 0)::numeric as gross_revenue
  from public.pedidos_operacionais pedido
  where pedido.data_venda is not null
    and pedido.data_venda >= (p_period_start::timestamp at time zone 'America/Sao_Paulo')
    and pedido.data_venda < (p_period_end::timestamp at time zone 'America/Sao_Paulo')
    and pedido.situacao <> 'cancelado'::public.pedido_status
  group by 1
  order by 1;
$rule02$;

revoke all on function public.get_pricing_monthly_revenue(date, date) from public, anon, authenticated;
grant execute on function public.get_pricing_monthly_revenue(date, date) to service_role;

drop function if exists public.search_produtos_paginated(
  numeric, text, text[], boolean, text, text, text, numeric, numeric, text, integer, integer, text, text
);
drop function if exists public.search_produtos_resumo(
  numeric, text, text[], boolean, text, text, text, numeric, numeric, text
);
drop function if exists public.search_produtos_paginated(
...
as $rule02$
  select case
    when p_custom_price is not null then round(p_custom_price * 100) / 100
    when 1 - (p_tax_rate + coalesce(p_ml_fee, 0.15)) <= 0 then null
    else round(greatest(
      (
        coalesce(p_cost, 0)
        + coalesce(p_shipping, 0)
        + coalesce(p_cost, 0) * case
          when coalesce(p_cost, 0) <= 400 then 0.15
          when coalesce(p_cost, 0) <= 1000 then 0.20
          else 0.25
        end
      ) / (1 - (p_tax_rate + coalesce(p_ml_fee, 0.15))),
      (
        coalesce(p_cost, 0)
        + coalesce(p_shipping, 0)
        + case
          when coalesce(p_cost, 0) <= 400 then 20
          when coalesce(p_cost, 0) <= 1000 then 60
          else 150
        end
      ) / (1 - (p_tax_rate + coalesce(p_ml_fee, 0.15)))
    ) * 100) / 100
...
revoke all on function private.rule_02_projected_price(numeric, numeric, numeric, numeric, numeric) from public, anon, authenticated;
...
begin
  if p_tax_rate is null or p_tax_rate < 0.04 or p_tax_rate >= 1 then
    raise exception 'p_tax_rate must be between 0.04 and 1 (exclusive)'
      using errcode = '22023';
  end if;
  ...
  base as (
    select
      ...
      private.rule_02_projected_price(
    ...
    select
      ...
      case
        when filtered.ml_status_operacional = 'sem_anuncio' then null
          ...
          - (coalesce(filtered.display_price, 0) * p_tax_rate)
    ...
    select
      ...
      case
        when b.ml_status::text = 'sem_anuncio' then null
          ...
          - (coalesce(b.display_price, 0) * p_tax_rate)
