...
async function assertDraftGtinIsUnique(productId,attributes) {
 const gtin=String(attributes?.find(a=>a.id==='GTIN')?.value_name||'').replace(/\D/g,'');
 if(!gtin)return;
 const matches=await checked(db.from('produtos').select('id,sku,ml_item_id').eq('gtin',gtin).neq('id',productId));
 if(matches.length)throw Error('GTIN_EM_OUTRO_CADASTRO_'+matches.map(p=>p.sku).join(','));
}
...
async function prepare() {
 ...
 const pending=reviews.filter(review=>review.status==='APTO_PREPARACAO'&&review.categoryReview?.version===1&&!results.some(r=>r.sku===review.sku&&r.status==='PREPARADO'&&JSON.stringify(r.categoryReview)===JSON.stringify(review.categoryReview)&&JSON.stringify(r.upscaylImages||{})===JSON.stringify(review.upscaylImages||{})&&r.draft?.familyName===review.title&&r.draft?.description===review.description&&JSON.stringify(r.draft?.attributes)===JSON.stringify(review.attributes)));
 ...
 await Promise.all(Array.from({length:Math.min(4,pending.length)},async()=>{while(next<pending.length){
  ...
  await assertDraftGtinIsUnique(p.id,review.attributes);
  ...
  const warrantyInput={evidence:[{origin:'GARANTIA_FORNECEDOR',productId:p.id,gtin:p.gtin||null,offerId:o.id,duration:Number(v.tempo_garantia),unit:'dias',source:`DSLite /v1/CrossDocking/Catalogo/${o.dslite_fornecedor_id}/${o.dslite_produto_id}:tempo_garantia; inspection.json`,observedAt:row.liveOffer.at}]};
  ...
  for(const original of selectedImages){
   ...
   let bytes=Buffer.from(await r.arrayBuffer()),meta=await require('sharp')(bytes).metadata();
   let contentType=r.headers.get('content-type');
   const sourceHash=crypto.createHash('sha256').update(bytes).digest('hex');
   const upscale=review.upscaylImages?.[original];
   if(upscale){
    if(upscale.visuallyReviewed!==true||upscale.sourceHash!==sourceHash)throw Error('UPSCAYL_ORIGEM_OU_REVISAO_DIVERGENTE_'+JSON.stringify({url,expectedHash:upscale.sourceHash,sourceHash,visuallyReviewed:upscale.visuallyReviewed}));
    bytes=fs.readFileSync(upscale.path);
    if(crypto.createHash('sha256').update(bytes).digest('hex')!==upscale.outputHash)throw Error('UPSCAYL_RESULTADO_DIVERGENTE');
    meta=await require('sharp')(bytes).metadata();
    if(meta.format!=='png')throw Error('UPSCAYL_FORMATO_INVALIDO');
    contentType='image/png';
   }
   if(!meta.width||!meta.height)throw Error('IMAGEM_DIMENSAO_INVALIDA');
   const smallSourceImage=Math.max(meta.width,meta.height)<500;
   ...
   if(existing.ok){
    ...
    const upload=await db.storage.from('product-images').upload(object,bytes,{contentType,upsert:false});
   ...
   images.push(publicUrl);imageEvidence.push({original,url:publicUrl,hash,width:meta.width,height:meta.height,...(upscale?{upscayl:upscale}:{})});
   if(IS_EVOLUSOM&&review.catalogProductId===null){
    const token=(await checked(db.from('integracoes').select('access_token').eq('tipo','mercadolivre').single())).access_token;
    const form=new FormData();form.append('file',new Blob([bytes],{type:contentType}),`${p.sku}.${meta.format==='jpeg'?'jpg':meta.format}`);
    ...
    if(!upload.ok||typeof uploaded.id!=='string'||!uploaded.id)throw Error('IMAGEM_UPLOAD_ML_'+upload.status+': '+JSON.stringify({message:uploaded.message,error:uploaded.error,cause:uploaded.cause}));
    const mlDimensions=String(uploaded.max_size||'').split('x').map(Number);
    if(mlDimensions.length!==2||!mlDimensions.every(n=>Number.isFinite(n)&&n>0))throw Error('IMAGEM_DIMENSAO_ML_INVALIDA_'+uploaded.max_size);
    if(Math.max(...mlDimensions)<500)throw Error('IMAGEM_DIMENSAO_ML_INSUFICIENTE_'+uploaded.max_size);
...
async function batchEvents() {
 ...
 for(let offset=0;;offset+=500){
  const page=await checked(db.from('pricing_events').select('*').contains('payload',{batchId:BATCH}).order('created_at').order('id').range(offset,offset+499));
  events.push(...page);if(page.length<500)return events;
 }
}
async function assertBatchReady() {
 const {assertCatalogExpansionCanAdvance}=require('../src/lib/ml/catalog-expansion.ts');
 assertCatalogExpansionCanAdvance(await batchEvents(),BATCH);
}
async function execute() {
 await assertBatchReady();
 const excludedProducts = new Set((await batchEvents()).filter(e => e.event_type === 'CATALOG_EXPANSION_SAFETY_STOP_RESOLVED' && e.payload?.outcome === 'EXCLUDED').map(e => e.produto_id));
...
for(const c of prepared){if(!SKUS.includes(c.sku)||excludedProducts.has(c.productId))continue;const reviewed=reviewRows.find(r=>r.sku===c.sku);if(!reviewed||reviewed.status!=='APTO_PREPARACAO'||reviewed.title!==c.draft.familyName||JSON.stringify(reviewed.attributes)!==JSON.stringify(c.draft.attributes))continue;await assertBatchReady();const completed=await checked(db.from('pricing_events').select('id,ml_item_id').eq('produto_id',c.productId).eq('event_type','CATALOG_EXPANSION_VALIDATED').contains('payload',{batchId:BATCH,...(c.replacementAuthorizationId?{replacementAuthorizationId:c.replacementAuthorizationId}:{})}).order('created_at',{ascending:false}).limit(1).maybeSingle());if(completed){await target(c,completed.ml_item_id);continue;}if(results.some(r=>r.sku===c.sku&&r.stage==='PUBLICACAO_PENDENTE'&&r.preparationId===c.preparationId))throw Error('RECONCILIACAO_PENDENTE_'+c.sku);const pending=await checked(db.from('pricing_events').select('id').eq('produto_id',c.productId).eq('event_type','CREATE_REQUESTED').contains('payload',{batchId:BATCH,...(c.replacementAuthorizationId?{replacementAuthorizationId:c.replacementAuthorizationId}:{})}).limit(1));if(pending.length)throw Error('RECONCILIACAO_PENDENTE_'+c.sku);
await assertDraftGtinIsUnique(c.productId,c.draft.attributes);
const simulation=await app('/api/pricing/simulate','POST',{productId:c.productId,categoryId:c.draft.categoriaId,listingType:c.draft.listingType,objective:'target'});if(!simulation.ok||!simulation.data.success){results.push({sku:c.sku,stage:'SIMULACAO',response:simulation});save('execution.json',{results});throw Error('SIMULACAO_PENDENTE_'+c.sku);}
...
if(!response.ok||!event)throw Error('PUBLICACAO_PENDENTE_'+c.sku);
await target(c,event.ml_item_id);
}
}
...
async function verifyPublicationTarget(app,c,itemId) {
 ...
 const atTarget=m=>m?.result>0&&m.margin+1e-10>=m.band.target&&m.margin<=m.band.limit&&m.fee.source==='ml_live'&&m.shipping.source==='ml_live';
 const token=(await checked(db.from('integracoes').select('access_token').eq('tipo','mercadolivre').single())).access_token;
 ...
 async function sim(body){await assertBatchReady();const r=await app('/api/pricing/simulate','POST',{productId:c.productId,itemId,...body});if(!r.ok||!r.data.success)throw Error('SIMULACAO_ALVO_'+JSON.stringify(r.data));return r.data;}
...
async function report() {
 const events=await batchEvents(),products=await checked(db.from('produtos').select('id,sku,nome,ml_item_id,ml_status').in('sku',SKUS));
 if(products.length!==SKUS.length)throw Error('MANIFESTO_PRODUTOS_INCOMPLETO');
 const links=[];
 for(let offset=0;offset<products.length;offset+=100)links.push(...await checked(db.from('anuncios_ml').select('produto_id,ml_item_id,status,preco_ml').in('produto_id',products.slice(offset,offset+100).map(p=>p.id))));
 const created=events.filter(e=>e.event_type==='CREATED_REMOTE'&&e.ml_item_id),latest=new Map();for(const e of created)latest.set(e.produto_id,e.ml_item_id);
 const ids=[...new Set([...products.map(p=>p.ml_item_id),...links.map(l=>l.ml_item_id),...latest.values()].filter(Boolean))];
 const token=(await checked(db.from('integracoes').select('access_token').eq('tipo','mercadolivre').single())).access_token,remote=new Map();let cursor=0;
 await Promise.all(Array.from({length:4},async()=>{while(cursor<ids.length){const id=ids[cursor++];const response=await fetch('https://api.mercadolibre.com/items/'+id,{headers:{Authorization:'Bearer '+token},signal:AbortSignal.timeout(25000)});if(!response.ok)throw Error('RELATORIO_READBACK_'+id+'_'+response.status);const item=await response.json();remote.set(id,{itemId:id,status:item.status,subStatus:item.sub_status??[],deleted:item.sub_status?.includes('deleted')??false,categoryId:item.category_id,sku:item.seller_custom_field,price:item.price,shippingMode:item.shipping?.mode,catalogListing:item.catalog_listing});}}));
 const rows=SKUS.map(sku=>{const p=products.find(p=>p.sku===sku),local=links.filter(l=>l.produto_id===p.id);const items=[...new Set([p.ml_item_id,...local.map(l=>l.ml_item_id),latest.get(p.id)].filter(Boolean))].map(id=>remote.get(id)),current=items.filter(i=>!i.deleted),issues=[];
  if(current.length>1)issues.push('MULTIPLOS_ANUNCIOS');
  for(const i of current){if(i.sku!==sku)issues.push('SKU_DIVERGENTE');if(p.ml_item_id!==i.itemId||!local.some(l=>l.ml_item_id===i.itemId))issues.push('VINCULO_DIVERGENTE');}
  if(items.some(i=>i.deleted&&(p.ml_item_id===i.itemId||local.some(l=>l.ml_item_id===i.itemId))))issues.push('VINCULO_EXCLUIDO');
  return{sku,productId:p.id,title:p.nome,hadCreatedListing:latest.has(p.id),status:current.length===0?'sem_anuncio':current.length===1?current[0].status:'multiplos',items,issues};});
 let batchBlocked=false;try{require('../src/lib/ml/catalog-expansion.ts').assertCatalogExpansionCanAdvance(events,BATCH);}catch{batchBlocked=true;}
 const summary={batchId:BATCH,at:new Date().toISOString(),manifestProducts:SKUS.length,productsWithCreatedListing:latest.size,replacementListingsCreated:created.filter(e=>e.payload?.replacementAuthorizationId).map(e=>e.ml_item_id).filter((id,n,a)=>a.indexOf(id)===n).length,productsWithoutNewListing:rows.filter(r=>!r.hadCreatedListing).length,awaitingReplacement:rows.filter(r=>r.hadCreatedListing&&r.status==='sem_anuncio').length,currentlyWithoutListing:rows.filter(r=>r.status==='sem_anuncio').length,active:rows.filter(r=>r.status==='active').length,paused:rows.filter(r=>r.status==='paused').length,underReview:rows.filter(r=>r.status==='under_review').length,linkIssues:rows.filter(r=>r.issues.length).length,batchBlocked};
 save('resultado-parcial.json',{summary,rows});
 const escape=v=> '"'+String(v??'').replaceAll('"','""')+'"';
 fs.writeFileSync(path.join(dir,'resultado-parcial.csv'),[['SKU','Produto','Estado atual','Anúncios','Pendências'],...rows.map(r=>[r.sku,r.title,r.status,r.items.filter(i=>!i.deleted).map(i=>i.itemId).join('|'),r.issues.join('|')])].map(row=>row.map(escape).join(',')).join('\n')+'\n');
 fs.writeFileSync(path.join(dir,'resultado-parcial.md'),`# ${BATCH} — situação atual\n\nConferência: ${summary.at}. Manifesto: ${summary.manifestProducts} produtos.\n\n- ${summary.productsWithCreatedListing} produtos tiveram anúncio criado; ${summary.replacementListingsCreated} substitutos não contam como produtos novos.\n- ${summary.active} ativos, ${summary.paused} pausados e ${summary.underReview} em análise pelo ML.\n- ${summary.productsWithoutNewListing} nunca publicados no lote; ${summary.awaitingReplacement} reposições pendentes; ${summary.currentlyWithoutListing} sem anúncio atual.\n- Divergências de vínculo: ${summary.linkIssues}. Lote bloqueado: ${summary.batchBlocked?'sim':'não'}.\n\nEstados remotos não comprovam correção de categoria. Revisão e impedimentos individuais: recuperacao-anuncios.json. Histórico preservado nos eventos; manifesto não foi reescrito.\n`);
 console.log(summary);
}
