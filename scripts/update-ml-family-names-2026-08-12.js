...
async function fetchLiveItems(ids) {
  ...
  for (let index = 0; index < ids.length; index += 20) {
    ...
    const response = await mlRequest(`/items?ids=${batch.join(',')}&attributes=id,title,family_name,user_product_id,seller_id,seller_sku,seller_custom_field,attributes,status,sub_status,sold_quantity,available_quantity,catalog_listing,category_id`);
