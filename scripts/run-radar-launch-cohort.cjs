...
async function publish() {
 ...
 const gate=read(path.join(DIR,'gate_results.json'));
 ...
 let job=await checked(db.from('jobs').select('id,status').eq('dedupe_key',COHORT).maybeSingle());
 if(job?.status==='completo'){console.log(JSON.stringify({status:'COHORT_ALREADY_FINISHED',jobId:job.id}));return;}
 if(!job)job=await checked(db.from('jobs').insert({tipo:'sync_ml_listings_publish',status:'on_hold',dedupe_key:COHORT,total:rows.length,log:[]}).select('id').single());
 const stopped=await checked(db.from('pricing_events').select('id').eq('event_type','COHORT_SAFETY_STOP').eq('job_id',job.id).limit(1));if(stopped.length)throw Error('COHORT_SAFETY_STOP_PENDING');
 ...
 try {
  for(const row of rows) {
   const {sku,c}=row;let itemId=null;
   try {
    const previous=await checked(db.from('pricing_events').select('*').eq('dedupe_key',`${COHORT}:${sku}:requested`).maybeSingle());
    if(previous){const remote=await freshDuplicates(rt,c,inventory);save(sku+'-reconciliation.json',{at:now(),previousRequest:previous.id,remote});const done=await checked(db.from('pricing_events').select('*').eq('dedupe_key',`${COHORT}:${sku}:validated`).maybeSingle());if(done&&remote.length===1&&remote[0].id===done.ml_item_id&&remote[0].status==='active'&&readbackChecks(previous.payload.payload,remote[0],rt.account.userId).ok){results.push({sku,itemId:done.ml_item_id,status:'PUBLICADO_VALIDADO',reconciled:true,price:done.new_price});continue;}throw Error('PUBLICACAO_JA_SOLICITADA_RECONCILIAR_SEM_NOVO_POST');}
    const resolved=await pricing.resolvePricingProduct(db,c.product.id);c.product=resolved.product;c.offer=resolved.offer;
    const supplier=await supplierLive(db,c.offer);assertSupplier(c,supplier);
    const catalog=await ml('/products/'+c.catalog.id);if(!catalog.ok||catalog.data.status!=='active')throw Error('CATALOGO_INDISPONIVEL');
    ...
    if(duplicates.some(i=>['active','paused','under_review','not_yet_active'].includes(i.status)))throw Error('ANUNCIO_EXISTENTE_OU_REATIVACAO');
    ...
    const payload={...row.payload,price:memory.price,available_quantity:quantity};
    ...
    const approval=await event(c,'APPROVED','approval',{acknowledgeEstimates:true,authorization:'Ordem explícita: até 10; garantia fabricante 12 meses',memory:finalEval.memory},null,evaluationId,payload.price);
    save(sku+'-before-post.json',{at:now(),supplier,assessment,payload,description:row.description,memory:finalEval.memory,evaluationId,approvalId:approval.id,duplicates,quantity});
    await event(c,'RADAR_LAUNCH_REQUESTED','requested',{payload,approvalId:approval.id,sourceHash:crypto.createHash('sha256').update(JSON.stringify(payload)).digest('hex')},null,evaluationId,payload.price);
    let post;
    try{post=await ml('/items','POST',payload);}catch(e){save(sku+'-post-unknown.json',{at:now(),error:e.message});throw Error('POST_RESULTADO_INCONCLUSIVO_NAO_REPETIR');}
    save(sku+'-post-response.json',post);itemId=post.data?.id??null;
    await event(c,'RADAR_LAUNCH_RESPONSE','response',{status:post.status,response:post.data},itemId,evaluationId,payload.price);
    ...
    const desc=await ml('/items/'+itemId+'/description','POST',{plain_text:row.description});if(!desc.ok)throw Error('DESCRICAO_NAO_PERSISTIDA');
    ...
    if(remote.data.status==='paused'&&!(remote.data.sub_status??[]).length){const activation=await ml('/items/'+itemId,'PUT',{status:'active'});save(sku+'-activation.json',activation);remote=await ml('/items/'+itemId+'?include_internal_attributes=true');}
    ...
    const description=await ml('/items/'+itemId+'/description');if(!description.ok||description.data.plain_text!==row.description)throw Error('DESCRICAO_DIVERGENTE');
  ...
  await checked(db.from('jobs').update({status:complete?'completo':'on_hold',processados:results.filter(r=>r.itemId).length,progresso:complete?100:0,finished_at:complete?now():null,log:[{event:'radar_launch_result',timestamp:now(),payload:{cohort:COHORT,results}}]}).eq('id',job.id));
...
module.exports={COHORT,DIR,runtime,loadPricing,checked,save,read,buildPayload,assertSupplier,supplierLive,payloadValidated,readbackChecks,assertCohortSize};
