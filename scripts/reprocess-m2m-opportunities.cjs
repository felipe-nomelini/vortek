...
const root=path.resolve(__dirname,'../reports/oportunidades-ml-2026-09-05');const dest=path.resolve(__dirname,'../reports/m2m-pricing-radar-2026-09-05');
...
for(const candidate of candidates){
 ...
 const remoteFacts=identityFacts(catalogData?.attributes??[]);const localFacts=supplierIdentityFacts(offer??p,remoteFacts);
 ...
 rows.push({sku:p.sku,produto:p.nome,fornecedor:base.suppliers.find(s=>s.dslite_id===offer?.dslite_fornecedor_id)?.nome??null,custo:offer?.custo??null,estoque:stock,revisado_65:!!r,anterior:r?.classification??before?.viavel_comercialmente??null,fila:classification.queue,etapa:classification.stage,identidade:assessment.identity,conflito:assessment.state,economia:assessment.economy,demanda: demand,preco_competitivo:competitive,preco_piso:floor?.price??null,preco_alvo:target?.price??null,break_even:breakEven?.price??null,margem_competitiva_pct:competitiveMemory?.margin==null?null:competitiveMemory.margin*100,contribuicao:competitiveMemory?.result??null,tributo_pct:tax.rate*100,tributo_status:tax.status,recuperado_piso_10:restored,motivos:assessment.reasons.join('; '),revisao_identidade_previa:evidenceReview,recomendacao:classification.recommendation,fonte:identity.source,avaliado_em:asOf});
...
rows.sort((a,b)=>a.fila.localeCompare(b.fila)||(b.contribuicao??-Infinity)-(a.contribuicao??-Infinity)||a.sku.localeCompare(b.sku));
...
const counts=rows.reduce((a,r)=>(a[r.fila]=(a[r.fila]??0)+1,a),{});const summary={asOf,universe:rows.length,reviewed:rows.filter(r=>r.revisado_65).length,recoveredBelow10:rescued,queues:counts,tax,mlRequests:0,remoteMutations:0,scope:'Reclassificação das evidências existentes; aprovação exige cotação viva e validação de pendências'};
