...
- A resposta real de validação veio em `message.codigo_pedido`: a Evolusom exige número, e rejeitou `BNT-2000018567229898`. Uma nova tentativa com `codigo_pedido` numérico `2000018567229898`, a mesma reserva e a mesma `data_pedido` recebeu HTTP 200. A resposta possui `status`, `data` e `message`, em formato diferente do exemplo do Swagger; como o número interno não foi extraído nessa execução, a reserva está em `uncertain` e o Bentevi ainda mostra a compra como não criada. O pedido não deve ser reenviado até a conferência e vinculação desse número.
