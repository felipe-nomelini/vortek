...
- Nas novas NF-e de venda geradas pelo fluxo de compra, o Código Produto (`cProd`) usa o SKU do item vendido no Bentevi (`pedido_itens.seller_sku`) para todos os fornecedores. O pedido triangular continua usando o código da oferta da Evolusom em `itens.cod_produto`; a associação com a linha fiscal aceita também as NF-e anteriores, cujo `cProd` era o código do fornecedor. Notas já autorizadas não são reemitidas.
...
- O Bentevi reserva em `compras.evolusom_request_code` um código numérico de oito dígitos para cada compra direta: `80000000` mais os últimos sete dígitos do número da venda. A derivação é estável para chamadas concorrentes, e a unicidade local é protegida por índice no banco; eventual colisão bloqueia a criação antes do POST. O mesmo código é enviado em `codigo_pedido` e preservado em retomadas. A escolha de oito dígitos segue o exemplo `80002719` do Swagger e evita o erro observado com 16 dígitos, mas o limite exato e o escopo de unicidade ainda dependem de confirmação da Evolusom. `data_pedido` representa a reserva da compra, gravada em `compras.data_criacao` e preservada em uma retomada; não é a data da venda no ML nem a emissão da NF-e.
- A compra registra `evolusom_attempt_count`. Uma criação nova pode fazer no máximo duas chamadas automáticas, com o mesmo código e payload, quando há falha transitória ou resposta sem número remoto válido. Após esgotar essas chamadas, ou após uma tentativa manual sem confirmação, a interface mantém “Criar pedido” como ação principal; cada clique autorizado faz uma chamada controlada. Só um número remoto válido encerra a opção de criar e leva ao fluxo normal. A reserva não confirmada pode ter sido processada apesar de um HTTP 500: a documentação da Evolusom não garante idempotência, então a repetição pode duplicar o pedido remoto e exige conferência operacional quando o retorno for ambíguo.
- Uma rejeição HTTP 400 documentada pela Evolusom como erro de validação permite nova tentativa manual com a mesma reserva. O Bentevi mostra os campos rejeitados sem expor os valores retornados na interface. Falhas e estados incertos sem número remoto mantêm a opção de tentativa manual; não ocorre reenvio automático depois do limite de duas chamadas.
