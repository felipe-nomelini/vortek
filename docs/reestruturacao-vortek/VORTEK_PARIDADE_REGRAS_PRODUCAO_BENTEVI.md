...
| `NTF-04` | commit `dc60a2b` | etiqueta por destinatário → message ID e confirmação persistidos por chave | destinatário já confirmado não reenvia no retry | WhatsApp etiqueta | ID e confirmação por destinatário em `jobs.log`; falha no checkpoint interrompe o processamento | `EQUIVALENTE` | `BNT-PARITY-11` implementada; 43 testes aprovados; homologação pendente |
