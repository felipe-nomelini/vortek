...
**Publicação passiva de 16/09/2026:** SHA funcional `6bc74b31898c61f631f044f1b6cc04f697e680ca` enviado a `dev` e promovido por fast-forward a `bentevi-prod`. A migration `20260916234500`, SHA-256 `a76830ab79cab3a1ddf01aa0a77ce897f6e481afb40b5bb6af83da996f9f1eff`, foi aplicada transacionalmente em `.162` após backup verificável da definição anterior em `/tmp/oraculo-orc07-backup-rceMOs/supplier_oracle_confirm_before.sql` (SHA-256 `c56a71745be78cf2429296127444d0162e2db37410a9788dd74034c0083f8eb1`). Read-back: função projeta `supplier_payment_receipt_path`, `service_role` executa, cliente autenticado não; registry na versão esperada. O webhook oficial aceitou o deploy HTTP `200`; o chunk novo de Compras `00ya7jeviqnl-.js` foi servido em produção com hash idêntico ao build local (`5baa046b6bbec27d4df3254d6bc0966ba740f0bc5541742507fff13c8fdc0470`). Saúde/login `200`, Compras/Pedidos/Créditos `307` sem sessão, APIs protegidas `401`. Read-back produtivo: zero liquidações, zero itens, zero jobs e zero compras alocadas; 23 compras PIX pendentes no momento da segunda leitura, nenhuma `ready`. Nenhum PIX, comprovante, WhatsApp ou retomada real foi provocado. **Limites da evidência:** não houve acesso à configuração privada do Easypanel para comprovar diretamente a flag em runtime ou o SHA da imagem ativa; o chunk prova o código cliente publicado, não o SHA completo do processo. A ativação financeira continua proibida até esse preflight e o canário com compras reais elegíveis.

**Continuação em 24/09/2026 — desenvolvimento e preflight parcial:** a leitura somente leitura pela API produtiva da `.162` encontrou 20 compras PIX pendentes, zero liquidações e zero itens. A regra publicada antes desta correção omitia duas compras diretas Evolusom sem `dsid`: uma venda cancelada (R$ 73,90) e uma venda ativa (R$ 29,99). Com a mesma elegibilidade aplicada também às compras diretas, a fotografia passa a 10 compras prontas, 10 excluídas e PIX sugerido de R$ 2.990,47: Evolusom 6 prontas/8 excluídas, BKR1 3/1 e Vanral 1/1; o crédito disponível de Vanral era R$ 314,78. As dez exclusões correspondem a vendas canceladas. Valores e cadastros devem ser relidos antes da operação. A correção em `dev` inclui origem Evolusom na prévia e no núcleo transacional, retira a exceção do writer individual e impede retomada DSLite para compra direta. Os testes dirigidos e o ensaio SQL sintético com PGlite passaram, incluindo lote misto, crédito, repetição, cancelamento e vínculo inválido; `npm run validate`, `npm run build` e verificação de secrets passaram com Node 22. **Ainda não há aceite operacional:** não foram comprovadas a configuração privada do serviço, a identidade SQL/schema/migrations da `.162`, nem a imagem ativa; a migration nova não foi aplicada, o código novo não foi publicado, a flag não foi ligada e nenhum PIX ou canário foi executado.
...
| 16/09/2026 | ORC-07 — publicação passiva | SHA funcional `6bc74b31898c61f631f044f1b6cc04f697e680ca`; migration `20260916234500` | Testes direcionados, validação web, build e banco sintético aprovados; migration aplicada em `.162`, grants preservados; webhook `200`, chunk de Compras com hash igual ao build local, saúde/login `200`, rotas protegidas `401` ou `307`. Read-back: zero liquidações/itens/jobs/alocações, 23 PIX pendentes, zero `ready` | **Pendente:** SHA da imagem e flag runtime não conferidos diretamente; sem candidato para canário, sem ativação financeira e sem aceite ORC-07 |
| 17/09/2026 | Simplificação da elegibilidade do ORC-07 | SHA funcional `921c936b8392f0067cc6c9a05e5633650d16460e`; migration `20260917130000`, SHA-256 `82f3af18fed3003d2065afafadebc61774ccd04b6f5a21f9f18451a9a1194640` | 29 testes direcionados, `npm run validate`, build, varredura de secrets e `git diff --check` aprovados. Função anterior salva em backup privado verificável; migration aplicada em transação curta à `.162`, registry e grants conferidos. Easypanel `cmu5w353e008407ov7tzhdx0d` concluiu; commit ativo e digest da imagem conferidos. Health/login `200`, Compras `307` e API protegida `401` sem sessão. | Regra e interface publicadas sem marcar compras como `ready`. Writer continua desligado; zero liquidações, itens ou casos de cancelamento abertos. Entre as cinco compras da imagem, `#391293` permanece sem venda vinculada e `#405861` tem venda cancelada; as outras três deixaram de ter os bloqueios removidos. Integração PostgreSQL sintética não executada neste ambiente sem Docker; canário autenticado e ORC-07 integral continuam pendentes. |
| 17/09/2026 | Correção do vínculo da compra `#391293` | SHA funcional `8815e35a300a7076d0523c7ff6e054aca14e52b6`; sem migration | Em `.162`, compra e venda tinham a mesma NF-e, sem outro par ou desvinculação manual. Backup privado da linha anterior em `/tmp/bentevi-dslite-391293-before.json` (SHA-256 `dd504b5675351816ba0de163b080fcb2459ff1b64458781cb2334d7bbcfdfd0e`). Atualização condicionada alterou só a venda `2000017647133832`; read-back confirmou `dslite_id=391293` e status DSLite copiado da compra. A mesma regra de elegibilidade do preview, aplicada aos dados atuais, retornou zero motivos de exclusão. 41 testes direcionados, `npm run validate`, build, varredura de secrets e `git diff --check` aprovados. `dev` e `bentevi-prod` receberam o mesmo SHA; Easypanel `cmu5wo5o4008m07ov3tmleh1m` terminou `done`, serviço ativo aponta para o SHA e processo reiniciou. Health/login `200`, Compras `307`, APIs protegidas `401`. | A compra não deve mais aparecer em “Compras fora do fechamento”; o PIX permanece pendente e sem alocação. Zero liquidações e itens criados. A causa exata da falha original de julho não pôde ser provada; o fluxo atual passou a confirmar a gravação do vínculo. ORC-07 financeiro continua pendente. |

## 9. Próxima ação permitida

**ORC-07 continua em andamento.** A exigência de classificação manual foi retirada do fechamento. A flag de escrita permanece desligada; a primeira liquidação real depende do gate financeiro próprio, com acompanhamento humano. ORC-08 aguarda a estabilização da ORC-07.
