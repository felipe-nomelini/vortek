...
**Situação vigente (08/09/2026):** V2-15 operacional publicado em DEV (`982be7b2`) e aceite inicial D20 concedido pelo usuário; marco 2 encerrado no recorte inicial, sem fechar V2-15/D20 integrais. [Evidências e aceite](evidencias/BNT-PRICING-V2-15-operacional-validacao.md#aceite-inicial-do-usuario--08092026). AI-00 [concluída para o piloto individual](evidencias/BNT-AI-00-validacao.md#fechamento-individual), somente assinatura ChatGPT e `gpt-6-astra/low`. Próxima ação: planejar AI-01. Chat/publicação e liberação do sócio permanecem separados e pendentes. Sequência atual na [fonte única de lançamento](VORTEK_ITEM_17_CHECKLIST_EXECUCAO.md#bentevi-em-operacao); PUB-GATE externo/ME2 no marco 6, capacidade produtiva no marco 5, sem liberação comercial ou produtiva.
...
| `BNT-AI-PROVIDER-01` | Garantia implementada; antes de fechar sua homologação | Avaliar modalidade suportada e montar piloto ChatGPT/Codex em DEV para extração da garantia; preservar Firecrawl, regras e limites; sem migrar demais consumidores ou antecipar chat | Resposta estruturada, evidência insuficiente, timeout, sessão expirada, limites e isolamento; uso da assinatura comprovado, sem API paga inadvertida; critérios completos no checklist |
...
| `BNT-CANON-PUB-GATE` | PRC-02A/03/04, QTY-01, trilha/exceções/grupo, CFL-01 a 04 e WARRANTY-01 validadas | Provar fluxo sugestão → preparação → aprovação → publicação → read-back, sem alterar atividade manual, duplicar anúncio ou perseguir prejuízo; recorte técnico do marco 1 concluído para sequência, aceite autenticado/prova externa no marco 6 | Produto simples, kit, ativo já anunciado, candidato a reativação, par de catálogo, prejuízo, dados inconclusivos, garantia ausente, retry/duplo clique e mudança de custo após aprovação |

O gate de publicação deve usar primeiro testes de contrato com respostas controladas e fixtures sanitizadas. Neste worktree, qualquer prova externa continua limitada à conta/item DEV e autorização específica, sem dados produtivos graváveis. Para o lançamento, a decisão do usuário de 08/09/2026 transfere o aceite autenticado e a prova externa ao marco 6, no workspace `vortek-prod` e release autorizado; permite avançar a V2-15 operacional pelo recorte técnico concluído, sem exigir novo produto de teste. Capacidade produtiva deve ser preparada/testada no marco 5. A prova não realizada permanece pendência do gate integral, nunca teste aprovado; demais validações de cada ação continuam obrigatórias. [Critérios e sequência de lançamento](VORTEK_ITEM_17_CHECKLIST_EXECUCAO.md#bentevi-em-operacao).
...
| PUB-GATE | Depender explicitamente de PRC-04, CFL-01/02/03/04, V2-04/05/06/13, QTY-01 e WARRANTY-01 validados. Provar sugestão → preparação → confirmação → publicação → read-back com produto simples, kit, anúncio existente e reativação. Prova neste worktree somente conta/item DEV; para lançamento, aceite autenticado/prova externa transferidos ao marco 6 produtivo autorizado, conforme recorte vigente, sem fechar o gate integral. Fixtures protegidas não viram alvos graváveis. Guard de transição só pode evoluir para a execução canônica controlada no escopo aprovado, nunca desaparecer sem substituto; preparar/testar capacidade produtiva no marco 5. |
...
## 19. M2M-CFL-01 — Contrato canônico de conflitos (07/09/2026)

Contrato C09 implementado em `src/types/commercial-conflicts.ts` e `src/services/commercial-conflicts.ts`, sem ativação nos consumidores. O classificador consolida avaliações de identidade, embalagem/quantidade, vínculo e economia. Estados, cobertura, evidências e motivos permanecem separados por dimensão; score/demanda não participam. Não substitui avaliadores, memória econômica ou autorização comercial.

**AS_IS → TO_BE:** `assessMlListingIdentity` ignora atributos ausentes e admite reconciliação de marca com SKU/GTIN; o sync usa `blockingConflicts` para reconciliar marca/bloqueio. Portanto, lista vazia não prova cobertura completa e não pode virar `SEM_CONFLITO` implicitamente. CFL-01 não muda esses fluxos; CFL-02 consolidará as verificações materiais, CFL-03 resolverá vínculos/grupos e CFL-04 avaliará economia a partir da memória canônica.
...
Migration `20260907213000` aplicada somente em `.162`, tipos gerados, SQL/concorrência e testes locais validados na entrega da garantia. `BNT-AI-PROVIDER-01` posteriormente validou login ChatGPT e extração real no piloto individual/local de Felipe, sem novas escritas de banco; [evidência](evidencias/BNT-AI-PROVIDER-01-validacao.md). Falta configurar Firecrawl/runtime local e homologar pesquisa/interface de WARRANTY-01, sem exigir OpenRouter no caminho Codex. Não confundir piloto de extração ou aceite anterior de override/liquidação com homologação comercial; guard continua bloqueado. Sequência: concluir garantia → V2-08 / CFL-04 — Buy Box econômica. O futuro chat reutilizará a evidência, mas uso compartilhado e implantação em servidor não estão homologados.
