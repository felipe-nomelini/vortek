...
- **Saneamento massivo de identidade — contenção P0 em 14/09/2026:** a migration `20260914180000_bnt_ml_catalog_identity_01` foi ensaiada com rollback, aplicada e registrada no Supabase produtivo `.162`. O guard final bloqueia escrita automática de preço sem identidade `SEM_CONFLITO` e fonte ML atual, preserva preço manual individual e invalida a liberação quando o snapshot material muda. As quatro tabelas privadas nasceram vazias, com RLS, dois triggers e zero grant direto a usuários; `produtos.ativo` permaneceu em 10.125 e a única saída de preço pendente continuou sendo a operação manual já existente. A aplicação de importação, dry-run, fila, manifesto e exports está validada em `dev`, mas a publicação web está bloqueada para não promover junto o commit BVF Storage ainda não autorizado. O dry-run dos 1.550 permanece bloqueado até o recebimento do CSV/XLSX original. Nenhum vínculo, preço, estoque ou anúncio ML foi alterado. [Evidências, gates e recuperação](evidencias/BNT-ML-CATALOG-IDENTITY-01-validacao.md).
