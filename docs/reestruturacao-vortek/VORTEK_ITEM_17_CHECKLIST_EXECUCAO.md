...
**Prioridade vigente — atualização de 17/09/2026:** o marco 7 do núcleo operacional foi fechado após prova do ciclo real, SHA ativo, release e executor único, mais confirmação expressa do responsável de que houve acompanhamento diário de 09 a 16/09, inclusive 12/09. Os registros diários completos não foram localizados; o aceite desse critério é declaratório e essa limitação permanece na [conferência do marco 7](evidencias/BNT-MARCO-7-OPERACAO-01-validacao.md). Continuar o read-back de cada operação real. Criação em lote, Assistente, Oráculo e automação própria permanecem em seus gates. O limite de custo de R$ 3.000 foi conferido no banco; a coorte associada ainda exige validação e aceite próprios.

**Simplificação geral — 19/09/2026:** a nova revisão operacional é acompanhada no [checklist geral de simplificação](VORTEK_BENTEVI_SIMPLIFICACAO_CHECKLIST.md). A execução permanece uma ação por vez; `BNT-SIMP-01` trata somente das contas proprietárias e não libera capacidades adiadas nem altera os gates de integrações.

### Trilha Oráculo de Fornecedores

A implementação da liquidação consolidada de fornecedores é acompanhada no checklist dedicado [VORTEK_ORACULO_FORNECEDORES_CHECKLIST.md](VORTEK_ORACULO_FORNECEDORES_CHECKLIST.md). `ORC-00` e `ORC-01 — Schema aditivo` foram aceitas em 16/09/2026. `ORC-02 — Estados e elegibilidade`, `ORC-03 — Núcleo financeiro transacional` e `ORC-04 — Pós-processamento e comunicação` foram aceitas tecnicamente em 16/09/2026, com SHAs promovidos, migrations aplicadas em `.162`, imagens comprovadas no contêiner ativo e read-back sem novos registros financeiros. `ORC-05 — Interfaces operacionais` foi aceita tecnicamente em 16/09/2026: SHA funcional `86c1883a954d6db3607b8c4a9247c7c34688dc05` promovido por fast-forward, migration `20260916230000` aplicada em `.162`, imagem ativa e smoke público confirmados. `ORC-06 — Cancelamentos e divergências` foi aceita tecnicamente em 16/09/2026: SHA funcional `1b29df036c7094ac45dd086515450364eae706c1` promovido por fast-forward, migration `20260916233000` aplicada em `.162`, imagem ativa, smoke público e read-back confirmados; 28 créditos históricos preservados e nenhum caso novo criado pela publicação. A flag de escrita segue desligada; o fluxo autenticado com dados reais ainda não foi exercitado. `ORC-07 — Ativação controlada` é a próxima ação técnica, em tarefa própria. Nenhum pagamento em lote, backfill financeiro, envio WhatsApp, retomada DSLite real ou ativação foi executado. Executar uma ação por tarefa e manter no checklist dedicado as evidências, SHAs, migrations, testes, publicação e aceite.

O painel de liquidação recebeu um refinamento visual passivo em 16/09/2026 (SHA `8b7546bbe5bd1fbf8e9d5baf68712109c105bdae`, imagem ativa e smoke público confirmados). O responsável informou aprovação visual da tela; a operação autenticada com dados reais continua sem prova. O refinamento não iniciou a ORC-07 nem liberou pagamentos consolidados.
...
| 7 — Bentevi em operação | Concluído no núcleo operacional em 17/09/2026, com aceite declaratório do acompanhamento diário | Nenhum neste recorte; registros diários completos não localizados | Manter read-back das sincronizações reais e seguir somente os gates próprios das capacidades adiadas | Venda, nota, compra, etiqueta, envio, entrega, SHA ativo, release e executor único comprovados; responsável confirmou acompanhamento de 09–16/09, inclusive 12/09; [evidência e limite](evidencias/BNT-MARCO-7-OPERACAO-01-validacao.md) |

**Correção pontual de anúncio existente — `BNT-ML-VTK018845-01`, 25/09/2026:**
o anúncio `MLB7602426324` foi corrigido no mesmo ID com foto do fornecedor,
descrição, família, modelo e atributos coerentes com a resistência Hikari HK
Plus CR30 220 V em pacote de 2 peças. Read-back do ML confirmou título gerado,
foto processada, moderação anterior ausente, status `active`, zero vendas,
preço `R$ 31,67` e quantidade `4`; o cadastro mestre e o snapshot local foram
conferidos. Esta correção não habilita criação de anúncios nem alteração de
preços. [Evidência, respostas e recuperação](evidencias/BNT-ML-VTK018845-01-validacao.md).
...
**Gate da operação comercial — simplificado em 17/09/2026:** preço, criação e republicação manuais exigem uma confirmação autenticada, uma operação auditada, claim único e read-back do anúncio escolhido. Propostas, segunda aprovação e bloqueios por margem/lucro foram removidos do fluxo manual. Seller/item corretos, valor positivo, estoque de publicação, atributos exigidos pelo ML, idempotência, concorrência e revalidação remota continuam obrigatórios. A automação de estoque/status permanece; criação em lote, automação própria de preço, `BNT-PRICING-V2-16`, `M2M-GATE` e `BNT-AI-GATE` seguem nos gates específicos.

**Evidência técnica da simplificação:** SHA `58ee446293c047e5b6c75796042f712657d4157f` enviado a `dev`, promovido por fast-forward a `bentevi-prod` e executado no serviço `local/bentevi-prod` (ação Easypanel `cmu4smqje003x07ov4gmafivh`, estado `done`). As migrations `20260917001000` e `20260917002000` foram aplicadas somente no Supabase produtivo `.162`. O arquivo privado de backup das 127 decisões antigas foi verificado por SHA-256 antes da retirada; o banco preserva as 127 decisões em `bentevi_archive`, 504 referências em eventos e três em itens de lote. Uma operação antiga pendente foi encerrada após leitura do preço já aplicado no ML, sem reenvio. Passaram 65 testes direcionados, `npm run validate`, `npm run build`, `npm run check:build-secrets` e `git diff --check`; health e login responderam `200`, rotas protegidas `401` sem sessão. A republicação do gabinete dependia da confirmação autenticada na interface e de read-back do novo ID naquele marco; nenhum novo anúncio foi enviado nessa mudança.

**Republicação real do gabinete — 17/09/2026:** o operador confirmou `VTK000980` por R$ 237,71, operação `3c76e075-b4bf-4851-9f90-329150375971`. O claim ficou preso em `prepared` porque a função SQL confundia a coluna `result` com a variável homônima (`42702`); o erro foi reproduzido em transação revertida, sem envio ao ML. O SHA `44a1c0c8706007ae6b6a17228204ccec0df550ef` foi publicado (Easypanel `cmu4tj4sy004l07ovamc81y36`, `done`) antes da migration `20260917003000`, aplicada e conferida em `.162`. A operação original então criou uma única vez `MLB7653041742`, mas a finalização tentou alterar a descrição oficial de um anúncio de catálogo. O SHA `6f1139862963c1ab38c508814b3ac9d99e4f2292` corrigiu essa conferência e foi publicado (Easypanel `cmu4tzpiz005007ov6mf55efy`, `done`). A execução presa do publicador foi marcada `erro` pelo mecanismo de recuperação após dez minutos; a fila da operação foi reaberta somente para read-back, sem novo POST. Em 01:12 UTC, banco e ML confirmaram o novo anúncio **ativo**, origem `MLB6711764286`, seller `3294514937`, preço **R$ 237,71**, estoque **1**, catálogo `MLB68889460`, vínculo do produto atualizado, operação `confirmed` e outbox `done`. O backup privado dos registros antes da reconciliação foi verificado. Passaram 30 testes direcionados, `npm run validate`, `npm run build`, `npm run check:build-secrets` e `git diff --check` para a correção final.
...
- **Alteração rápida do preço no catálogo — 23/09/2026:** `Revisar preço` abre um painel com prévia automática do valor digitado e um único botão de confirmação; o andamento é mostrado por anúncio na lista, sem modais adicionais. O caminho manual cota somente o preço escolhido e preserva os controles técnicos e o read-back do worker. O código foi publicado sem migration; smoke produtivo comprovou o novo chunk e os controles sem sessão. Não houve alteração real de preço nesta validação. [Evidências e limite da comprovação do SHA ativo](evidencias/BNT-CATALOG-FAST-PRICE-01-validacao.md).
- **Vínculo de catálogo no preço manual — correção produtiva em 14/09/2026:** a revisão de preço passou a aceitar o vínculo canônico consistente de `catalogo_ml_snapshot`, mesmo quando a linha derivada ainda não existe em `anuncios_ml`; proprietário conflitante continua bloqueado. A sincronização observada deixou de enviar a coluna inexistente `listing_type_id` ao snapshot, separou o instante econômico observado do `last_updated` remoto e agora explicita falhas fatais. O ciclo produtivo retomado terminou `7055/7055`, sem item falho, e o caso real `VTK017371` foi reconciliado sem alterar preço nem enfileirar publicação. [Evidências e recuperação](evidencias/BNT-CATALOG-PRICE-LINK-01-validacao.md).
- **Saneamento massivo de identidade — P0 concluída em 16/09/2026:** o guard, as migrations e o ledger estão ativos no Supabase produtivo `.162`. A release final `84069a6a` revalidou e liberou 27 drifts de título com Rodrigo como ator, sem executar testes BVF nem incluir Video Factory no delta. O estado final é 1.526 `SEM_CONFLITO`, 22 `CONFLITO_CONFIRMADO` e 2 `PENDENCIA_VALIDACAO`, fechando `1.550 = 1.526 + 22 + 2`. Readback e snapshots confirmaram zero alteração em preço, vínculo, status de anúncio, `produtos.ativo`, `custom_price` ou estoque, além de zero relink e zero repricing. Os 24 casos não liberados continuam bloqueados para pricing. [Evidências, gates e recuperação](evidencias/BNT-ML-CATALOG-IDENTITY-01-validacao.md).
- **Origem fixa de fornecedor dos kits — correção produtiva em 15/09/2026:** os 1.021 kits ativos simples passaram a usar exclusivamente a oferta do fornecedor configurado para capacidade, custo, pricing, CMV, preview e compra DSLite, mantendo estoque interno físico como prioridade. O ciclo BKR1 recalculou 338 kits e encerrou com zero divergência; 17 publicações de quantidade/status foram confirmadas sem erro e sem preço. A compra `409605`, criada pelo processo antigo durante o primeiro build para a venda original, permaneceu em MKS e não foi cancelada ou recriada sem decisão operacional específica. [Evidências e recuperação](evidencias/BNT-KIT-SUPPLIER-SOURCE-01-validacao.md).
- **Código do produto fiscal dos kits — contrato vigente desde 21/09/2026:** a NF-e de venda usa o SKU Bentevi de `pedido_itens.seller_sku` para todos os fornecedores, inclusive kits; a quantidade do kit continua expandida para o componente. A descrição de 15/09 sobre o código DSLite no XML é histórica e foi substituída por essa regra. A compra DSLite deve reconhecer tanto o SKU Bentevi das notas atuais quanto o código DSLite das notas anteriores, sem reemitir notas autorizadas, e preservar a oferta do fornecedor configurado do kit.
- **Confirmação posterior do PIX ao fornecedor — ajuste e primeira prova produtiva em 15/09/2026:** ao alcançar o bloqueio PIX depois da criação da compra DSLite, Vendas passou a oferecer `Pagar agora` ou `Pagar depois e continuar`. O adiamento exige compra pré-paga pendente já vinculada, reutiliza o mesmo DSID e segue com transportadora e etiqueta real/provisória conforme as regras existentes; a confirmação posterior conserva comprovante e WhatsApp atuais. A exceção da BKR1 não foi alterada. O SHA funcional `7592fd0c` passou por 42 testes direcionados, suíte integral, lint, tipos, build, varredura de segredos e smoke produtivo. O complemento `3c7b1cb2` habilitou somente para a MKS (`115`) o mesmo PDF genérico da BKR1, com origem própria e sem herdar pagamento, formato térmico ou WhatsApp. Na venda real `2000018469395176`, a leitura válida do ML encontrou `ready_to_ship/invoice_pending`; o job `18f27413-f336-451a-9b2e-6bd5500be1e0` preservou a única compra e o DSID `409605`, enviou a etiqueta genérica, definiu Correios e terminou 11/11. O PIX de R$ 960,00 continuou pendente, sem comprovante, confirmação ou mensagem de WhatsApp. [Evidências e recuperação](evidencias/BNT-DSLITE-DEFERRED-PIX-01-validacao.md).
...
**Controle documental desta decisão:** somente planejamento/checklist; sem implementação funcional, conexão ML, banco, migration ou liberação de produção. As evidências técnicas e aceites históricos permanecem datados. A atualização reconhece o recorte técnico já executado do marco 1 para liberar sequência, sem inventar nova validação nem fechar seu aceite operacional/PUB-GATE integral.

**Histórico — validação documental do recorte inicial de lançamento, antes da transferência da prova PUB-GATE (08/09/2026):** branch `dev`, árvore inicialmente limpa; seis documentos reconciliados. Conferência automatizada confirmou sete marcos únicos e pendentes, âncora e cinco referências ao recorte, 14 etapas futuras mantidas pendentes e ausência de alteração em `AGENTS.md`, `.rules` e `.gitignore`. `npm run validate` (lint/typecheck) e `git diff --check` passaram. Build, homologação visual e consulta a contratos externos: **N/A nesta tarefa exclusivamente documental**, sem modificar comportamento de biblioteca/API; reconfirmação oficial permanece obrigatória nas ações técnicas futuras. Sem commit, push, deploy ou acesso à infraestrutura nesta tarefa.
...
- [x] Antes da ativação operacional da regra Evolusom, configurar `EVOLUSOM_OFFICIAL_LABEL_ADDITIONAL_PHONE` no servidor e publicar o código no ambiente autorizado; não usar contato real em homologação.
...
**Contrato e isolamento:** os contratos existentes de `GET /api/tv/live` e `GET /api/tv/metrics`, suas fontes reais e a proteção por `tv.read` permaneceram inalterados. Na entrega original não houve migration, escrita no Supabase, alteração de integração externa, dependência nova ou uso dos três áudios `.m4a`, que permaneceram fora daquele escopo.
...
**Ajuste produtivo de sons em `2026-09-10`:** os três áudios `.m4a` fornecidos pelo usuário foram versionados e adicionados ao som de venda já existente. A cada nova venda, a TV sorteia uniformemente um dos quatro sons, permitindo repetição consecutiva; antes de tocar o escolhido, interrompe e reinicia qualquer áudio de venda anterior. Os quatro elementos são pré-carregados, o controle de som recarrega todos eles e o aviso de perguntas continua usando exclusivamente `ala-nem-vou-ler-ines-brasil.mp3`. O carregamento inicial continua sem disparar celebração ou áudio. Não houve mudança de API, banco, integração externa ou dependência. O commit funcional `234e151c` passou por 7 cenários direcionados, `npm run validate`, `npm run build` com Next.js `16.3.3`, `npm run check:build-secrets` e `git diff --check`; foi confirmado no mesmo SHA em `origin/dev`, `origin/bentevi-prod` e `local/bentevi-prod`. Em `app.bentevi.shop`, health e proteções de acesso passaram, e um smoke autenticado somente leitura confirmou `200` nos quatro assets, com `audio/mpeg` no MP3 e `audio/mp4` nos três M4A. O rollback específico é reverter `234e151c`, promover o SHA da reversão e reimplantar `local/bentevi-prod`.

**Correção de contagem e aproveitamento da tela em `2026-09-10`:** o total de anúncios na TV estava limitado a `1.000` porque `GET /api/tv/metrics` carregava linhas de `anuncios_ml` sem paginação e fazia a soma no JavaScript, ficando sujeito ao limite de resposta do PostgREST. O commit funcional `5b00d2d6` substituiu essa leitura pelas contagens exatas do banco para total, ativos, pausados e catálogo ativo; a contagem de catálogo vencedor já exata foi preservada. Uma falha em qualquer contagem agora invalida a atualização completa, permitindo que a tela mantenha o último snapshot válido, e os números grandes são formatados em `pt-BR`. Não houve chamada adicional ao Mercado Livre, migration ou escrita no Supabase.

No mesmo ajuste, monitores a partir de `1451 × 900` passaram a distribuir a altura disponível entre resultados, tendências e atividade, com crescimento da área de vendas/perguntas e tipografia ampliada para títulos, subtítulos e textos operacionais. Permaneceram exatamente cinco vendas e três perguntas visíveis, assim como polling, tela cheia, sons e celebrações. Passaram 9 cenários direcionados, `npm run validate`, `npm run build` com Next.js `16.3.3`, `npm run check:build-secrets` e `git diff --check`. O SHA `5b00d2d69dd834fbee75b0fe1acc7423893754e3` foi confirmado em `origin/dev`, `origin/bentevi-prod` e no serviço `local/bentevi-prod`; a ação Easypanel `cmtvkuob200bg07mfeovc2r0g` terminou com `done`, e o health produtivo respondeu `200`. O smoke autenticado e somente leitura de `GET /api/tv/metrics` respondeu `200` com 5 vendas recentes e os totais exatos então vigentes: 7.017 anúncios, 4.585 ativos, 2.431 pausados, 1.366 em catálogo ativo e 305 ganhando catálogo. O rollback específico é reverter `5b00d2d6`, promover o SHA da reversão e reimplantar `local/bentevi-prod`.
...
**Pendência:** conferir visualmente a versão ampliada de `/tv` autenticada em `app.bentevi.shop`, inclusive em tela cheia, e confirmar os quatro sons durante novas vendas reais. O item permanece desmarcado até esse aceite.
...
**Correção das contagens de Vendas em `2026-09-22`:** o cartão Urgentes mostrava nove vendas enquanto a fila estava vazia porque o resumo e o Dashboard avaliavam urgência sem enriquecer as compras Evolusom, ao contrário da lista. As nove vendas sem `dslite_id` já tinham compra Evolusom; sete aguardavam etapas anteriores à liberação da etiqueta e duas tinham entrega da etiqueta real registrada. O resumo também lia somente os primeiros 1.000 pedidos para status e valores. O commit funcional `72629b74dffd7d0aeca5261cc3a6b90f7fbdbe67` unificou a projeção de compras e etiquetas para a lista, o resumo e o Dashboard, e passou a agregar todos os pedidos em lotes de 500, preservando a regra de urgência e a exclusão financeira das canceladas. Passaram 33 testes direcionados, `npm run validate`, `npm run build`, `npm run check:build-secrets` e `git diff --check`. O SHA foi confirmado em `origin/dev` e `origin/bentevi-prod`; o webhook Easypanel respondeu `200` e o processo produtivo reiniciou, com health/login `200` e as três APIs protegidas retornando `401` sem sessão. Leitura somente leitura do Supabase produtivo `.162` em 22/09 confirmou 1.608 vendas, 15 em preparação e 1.372 entregues. **Aceite produtivo ainda pendente:** a ação do Easypanel e o SHA exato do contêiner não puderam ser lidos diretamente; a comparação autenticada de cartão, lista e Dashboard também não pôde ser executada. Não houve migration, escrita no banco nem ação em integrações. Para reverter o código, criar commit de reversão, promover por fast-forward e reimplantar; não há rollback de banco.

**Correção produtiva do fluxo etiqueta genérica → WhatsApp em `2026-09-11`:** o erro era a projeção de uma tentativa de substituir na DSLite a etiqueta genérica já aceita: o HTTP `403` de pedido em status protegido era tratado como falha comum, apagava a evidência local anterior e mantinha `Retomar fluxo DSLite` como ação principal. O commit funcional `b9df4e32` passou a reconhecer somente essa resposta específica como etiqueta DSLite existente/protegida, preservar a etiqueta anterior, encerrar ações DSLite obsoletas e exigir a etiqueta real por WhatsApp quando ela estiver liberada. Outros erros continuam falhas; envio pendente não duplica ação e envio confirmado remove a venda de Urgente. Foram aprovados 72 testes direcionados, `npm run validate`, `npm run build` com Next.js `16.3.3`, `npm run check:build-secrets` e `git diff --check`; nenhum arquivo de `mobile/`, migration ou dado foi alterado. O SHA foi confirmado em `origin/dev`, `origin/bentevi-prod` e `local/bentevi-prod`; a ação Easypanel `cmtwgpd24000607nteh2mbr0r` terminou em `done`, sem erro terminal, e health/login produtivos responderam `200`. O read-back somente leitura no Supabase produtivo `.162` reconciliou cinco pedidos pelo `403` histórico e seis pela fonte genérica persistida: a venda `2000018324080338`, com WhatsApp já enviado, passou a `done` e saiu de Urgente; outras cinco vendas com etiqueta real liberada permaneceram Urgentes exclusivamente para envio por WhatsApp. O rollback é reverter `b9df4e32`, promover o novo SHA e reimplantar `local/bentevi-prod`; não há rollback de banco.

**Correção do link público da etiqueta em `2026-09-23`:** o link curto enviado ao fornecedor apontava para `app.bentevi.shop`, mas a rota pública redirecionava o arquivo para o endereço privado `192.168.1.162`. O commit funcional `84e783cf7baedc2925cc8a53f3c792e17cb35c2b` passou a entregar PDF e ZPL diretamente pela rota pública, preservando token, formato e links já enviados. Passaram 13 testes direcionados, `npm run validate`, `npm run build`, `npm run check:build-secrets` e `git diff --check`. O mesmo SHA foi confirmado em `origin/dev` e `origin/bentevi-prod`; o webhook Easypanel aceitou o deploy com HTTP `200`. No pedido BKR1 DSLite `411639`, o link curto continuou com `302` para `app.bentevi.shop`, e a resposta final mudou de `302` para o IP privado para `200` sem redirecionamento: `text/plain`, arquivo ZPL de 3.023 bytes com `^XA` e `^XZ`, igual ao tamanho registrado no pedido. Token inválido retornou `403`. O health mostrou reinício do processo e nenhum WhatsApp foi reenviado. A ação Easypanel e o SHA do contêiner não foram lidos diretamente; a nova rota foi confirmada pelo comportamento produtivo. Não houve migration nem alteração de dados. Para reverter, criar commit de reversão, promover o SHA e reimplantar `local/bentevi-prod`.

**Etiqueta real da MKS em ZPL em `2026-09-23`:** a venda `2000018580091068` recebeu PDF por WhatsApp porque a MKS não estava nos seletores térmicos. O commit funcional `8589fa64e073cb320387e17015b121ec0dc93def` incluiu a MKS no formato ZPL2 das etiquetas reais para as duas rotas DSLite e os dois fluxos WhatsApp; o PDF provisório permanece e pedidos DSLite protegidos continuam encaminhando a etiqueta real ao WhatsApp. Os testes cobriram ZPL salvo, PDF preexistente, download ZPL2, nome, MIME, link público e falha sem fallback para PDF. Passaram 44 testes direcionados, `npm run validate`, `npm run build`, `npm run check:build-secrets` e `git diff --check`. O mesmo SHA foi confirmado em `origin/dev` e `origin/bentevi-prod`; o webhook Easypanel respondeu `200`. Em `app.bentevi.shop`, o health retornou `success=true` e o uptime caiu de 1.526 para 31 segundos após o deploy; `/login` respondeu `200` e as APIs protegidas de pedidos e compras, `401` sem sessão. O SHA exato do contêiner e um envio MKS novo não puderam ser verificados por leitura externa; o aceite do formato produtivo fica pendente do próximo envio natural. A venda citada não foi reenviada ou reprocessada. Não houve migration, escrita no banco nem chamada de envio à DSLite ou ao WhatsApp durante a validação. Para reverter, criar commit de reversão, promover o novo SHA e reimplantar `local/bentevi-prod`.

**Ativação produtiva do destinatário adicional Evolusom em `2026-09-11`:** a causa do bloqueio era a ausência de `EVOLUSOM_OFFICIAL_LABEL_ADDITIONAL_PHONE` no ambiente de `local/bentevi-prod`, embora o worker já exigisse o segundo destinatário para etiquetas oficiais vinculadas ao fornecedor `133`. O contato aprovado foi recuperado da fonte privada já documentada, validado e acrescentado somente ao ambiente do serviço pela ação Easypanel `cmtwhqv6v001s07ntg9opcsyb`, sem exposição do valor. O commit `763680cb` tornou a chave obrigatória no preflight produtivo e validou o mesmo conjunto de caracteres e comprimentos aceito pelo runtime, mantendo o valor fora do resumo e dos logs. Passaram 21 testes direcionados, `npm run validate`, `npm run build` com Next.js `16.3.3` e 128 rotas, `npm run check:build-secrets` e `git diff --check`. A ação `cmtwhrngp001u07nt7mvs6cy0` terminou em `done`; o serviço confirmou `bentevi-prod` no mesmo SHA, configuração válida e health/login HTTP `200`. Por solicitação do responsável, nenhum job foi criado ou retomado e nenhuma mensagem de WhatsApp foi enviada; a nova tentativa permanece manual na interface. Não houve alteração de banco, migration, pedido ou etiqueta. O rollback do gate é reverter o commit e reimplantar; retirar a configuração com a regra atual ativa restaura o bloqueio seguro dos envios oficiais da Evolusom.

**Correção produtiva de lucro pendente em `2026-09-10`:** a causa dos lucros `R$ 0,00` em vendas novas era a combinação do default obrigatório `pedidos.lucro = 0` nos stubs recebidos por webhook com uma proteção que reconhecia somente `lucro_pendente_produto`. O commit funcional `517a7a4a` passou a gravar `null` explicitamente nos novos stubs, reconhece os cinco marcadores provisórios existentes e só substitui um zero provisório depois de concluir as evidências econômicas; valores históricos não nulos e zeros sem marcador permanecem preservados. A migration `20260910120000_bnt_order_profit_nullable.sql` foi ensaiada com `ROLLBACK`, aplicada e registrada no Supabase produtivo confirmado em `.162`, removendo apenas o default e o `NOT NULL` da coluna. Testes direcionados `27/27`, `npm run validate`, `npm run build`, `npm run check:build-secrets` e `git diff --check` passaram. O mesmo SHA foi confirmado em `origin/dev`, `origin/bentevi-prod` e no serviço `local/bentevi-prod`; a ação Easypanel `cmtvjp5ff00ak07mfe1ra09a2` terminou sem erro e o health produtivo respondeu `200` com Mercado Livre conectado.

O reparo de dados foi limitado às vendas `2000018384965428` e `2000018385394212`, únicas vendas novas comprovadamente presas no placeholder: backup privado de seus dois pedidos e dois itens foi criado antes da escrita, os placeholders foram convertidos para pendentes e cada pedido foi reconciliado pela rota produtiva em modo `cutover`, sem alerta externo. O read-back confirmou, respectivamente, lucros de `R$ 34,75` e `R$ 9,32`, CMVs de `R$ 179,90` e `R$ 50,50`, snapshot completo, um item e uma única linha por pedido. O total permaneceu em 1.491 pedidos; os outros 29 lucros históricos iguais a zero não foram alterados e não restou lucro nulo. Para recuperar somente os dados, usar o backup privado e conferir eventos posteriores antes de qualquer restauração; para desfazer o contrato, criar uma migration progressiva somente depois de eliminar lucros nulos e reverter o código por novo commit, sem restaurar cegamente o banco.
...
**Resultado da alteração de status em `2026-09-17`:** após a confirmação, a tela de resultado foi substituída por uma notificação que acompanha envio, conclusão ou falha. Uma sessão expirada agora recebe orientação para entrar novamente; falhas simples exibem seu motivo, sem detalhes técnicos. A tentativa mostrada pelo usuário para `VTK017472/MLB7601891176` não gerou operação de pausa na fila, e o produto permaneceu ativo com 48 unidades; sem a resposta HTTP daquela tentativa, sua causa exata não foi comprovada. O SHA `bd5c14ac453793b1c466147e9a3a213439b16fdb` passou em 16 testes direcionados, `npm run validate`, build, verificação de secrets e `git diff --check`; foi promovido para `bentevi-prod` e publicado pela ação Easypanel `cmu5ymrhe009i07ov46twfwwe`, concluída. O serviço ativo apontou para esse SHA; saúde/login `200`, Anúncios `307` e APIs sem sessão `401`. Nenhuma pausa real foi enviada durante esta validação; o clique autenticado na interface nova ainda não foi exercitado.

**Correção do pedido de pausa em `2026-09-17`:** a rota de status enviava `desired_price=570,68` junto com a pausa, apesar de `apply_price=false`. O gatilho de identidade do banco verifica qualquer preço não nulo; a função de leitura para `MLB7601891176` confirmou `ml_identity_price_write_blocked`, e nenhuma operação de pausa havia sido criada. A rota passou a enviar somente o status, sem preço ou quantidade; a notificação distingue falha de registro de falha após o envio. O SHA `5c62605b5f272bcc53f7949946dce998a824552e` passou em 41 testes direcionados, `npm run validate`, build, verificação de secrets e `git diff --check`; foi promovido para `bentevi-prod` e publicado pela ação Easypanel `cmu5z8a1z009z07ovaiq4e3in`, concluída. O serviço ativo apontou para esse SHA; saúde/login `200`, Anúncios `307`, APIs sem sessão `401`. O deploy não pausou o anúncio. O clique autenticado posterior criou a operação `3f743bb6-e3fd-40dd-a0c4-a6697563a227`, concluída em uma tentativa, com `desired_price` e `desired_quantity` nulos; o Mercado Livre confirmou `paused` com preço `570,68` e 48 unidades.

**Estado exibido na lista em `2026-09-17`:** após a pausa real, `produtos.ml_status` e `anuncios_ml.status` estavam pausados, mas o snapshot de catálogo ainda tinha `active` de 18h14. A busca priorizava esse valor antigo e exibia “Ativo”. O publicador agora atualiza o estado do snapshot a partir da leitura final do ML antes de marcar a operação como concluída. Após confirmar novamente `paused` no ML, somente o status do snapshot de `MLB7601891176` foi corrigido em `.162` de `active` para `paused`; `synced_at` e os demais dados foram preservados. A busca produtiva passou a devolver `observedStatus=paused`, `localStatus=pausado`. O SHA `24aa70bbce30925b820adb89ba20f0e0b3ce9c9d` passou em 21 testes direcionados, `npm run validate`, build, verificação de secrets e `git diff --check`; a suíte mais ampla teve 33 aprovações e duas falhas preexistentes em mocks de rotas de preço sem Zod, não relacionadas à mudança. O SHA foi promovido e publicado pela ação Easypanel `cmu5zux6c00ae07ovc965aq2p`, concluída; saúde/login `200`, Anúncios `307`, APIs sem sessão `401`. Read-back final: lista, produto, snapshot e Mercado Livre pausados; preço `570,68`, estoque `48`, outbox `done`. Sem pendência funcional neste recorte.
...
**Pendência:** nenhuma. A aba e o histórico administrativo foram aprovados visualmente pelo responsável em `2026-09-04`. Próxima ação liberada: `BNT-CFG-02 — Empresa e cadastro fiscal`.
...
**Mudança realizada:** a aba passou a se chamar `Empresa e fiscal` e foi dividida em identidade/contato, endereço fiscal estruturado, Simples Nacional e saúde do emissor. CNPJ numérico ou alfanumérico é normalizado e validado pelo contrato vigente; CEP, UF e código IBGE possuem validações coerentes, sem exigir complemento. CNPJ/endereço e parâmetros tributários exigem confirmação antes de salvar. RBT12, faixa e alíquotas calculadas permanecem somente leitura. Brasil NFe, ambientes, validação estrita, CFOPs e fuso são exibidos como contratos protegidos, não como opções livres. O início de atividade deixou de ter fallback no código, o seletor de provedor único foi removido e UF fiscal deixou de ser inferida do endereço legado nos fluxos afetados.

**Banco DEV:** a migration aditiva `20260904210000_bnt_cfg_02_company_fiscal.sql` teve destino resolvido e confirmado como `supabase-dev` em `192.168.1.162`, PostgreSQL `17.6`; foi ensaiada integralmente com `ROLLBACK` e depois aplicada em transação nesse mesmo destino. O read-back confirmou migration registrada, colunas estruturadas, constraints, índice singleton, RLS e bloqueio dos papéis cliente. Foi criado apenas o registro canônico ausente de `configuracoes`, preservando o início de atividade que já era usado pela regra existente; nenhum cadastro empresarial ou dado de produção foi copiado. O banco de produção em `.160` não foi acessado.

**Commit, testes e homologação:** `439e685` — `feat(configuracoes): estruturar cadastro empresarial fiscal`, enviado somente para `origin/dev`. Passaram 14 cenários direcionados, `npm run validate`, `npm run build` com Next.js `16.3.3`, 121 páginas/rotas e `git diff --check`. O deploy oficial do `vortek-erp-dev` foi aceito e a instância ativa confirmou `GIT_SHA=439e68514cfcab15b8a32e661f5241faa70b0c14`; health e login responderam `200`, `/configuracoes` respondeu `307` sem sessão e as APIs de Empresa e Fiscal responderam `401` sem sessão.
...
**Validação:** passaram os 6 testes direcionados do PDF de vendas e os 86 testes da suíte afetada de concretização, webhook, hidratação, shipment, pedidos, tracking, mobile, DSLite e PDF. A asserção textual que ainda esperava a assinatura anterior de `getOperationalUrgencyReasons` foi alinhada ao parâmetro configurável `delayedAfterMinutes`, sem alterar código funcional. `npm run validate`, `npm --prefix mobile run typecheck`, `npm run build` com Next.js `16.3.3`, `npm run check:build-secrets` e `git diff --check` passaram na implementação funcional; `npm run validate` e `git diff --check` passaram novamente após a correção do teste.
...
#### Classificação dos deltas de pricing — preparação do `BNT-PARITY-GATE`

**Situação:** ação documental concluída em `2026-09-05`. Referência DEV `0b04a09`; remoto `b6e1b17eba58f0ec80a3d16357ac7ab2409f56de`. Não houve implementação ou ativação do experimento.

- [x] conferir os cinco commits e 18 caminhos de `95941f1..b6e1b17` contra a política canônica e o código DEV;
- [x] registrar no catálogo 16 regras: 9 `INCORPORAR`, 1 `EQUIVALENTE`, 3 `SUBSTITUÍDA`, 3 `NÃO COPIAR`, com destino e evidência;
- [x] distinguir parâmetros/coorte D0 de requisitos permanentes e documentar que compensação não é transação atômica;
- [x] acrescentar os critérios de regressão nas ações V2 responsáveis, sem alterar a política ou a ordem de execução;
- [x] verificar hashes e tamanhos dos oito arquivos do manifest D0, sem extração do ZIP ou importação da coorte;
...
**Limites:** somente catálogo, plano V2 e este checklist; sem mudanças em código, APIs, tipos, migrations, configuração de runtime, banco, preços ou infraestrutura. Sem push/deploy. Build e homologação visual não aplicáveis. Rollback: reverter somente o commit documental desta ação, preservando as pendências anteriores.

**Próxima ação:** planejar e avaliar `BNT-PARITY-GATE`. Se não houver aceite explícito das lacunas encaminhadas, ele permanece bloqueado; `BNT-PARITY-FINAL`, delta de migrations e gate de autonomia continuam obrigatórios para release.
...
**Decisão posterior de 08/09/2026:** primeiro teste **somente para Felipe**; acesso do sócio será uma liberação separada. AI-00/01 concluídas e AI-02 ativada no DEV, com login e inferência reais. Não liberado para uso compartilhado ou produtivo. Manter somente assinatura ChatGPT, `gpt-6-astra/low`, sem API paga ou substituição automática. Concluir correção/validação semântica da consulta de vendas antes de encerrar AI-GATE. [Contrato canônico](VORTEK_BENTEVI_ASSISTENTE_CONTRATO.md) e [validação AI-02](evidencias/BNT-AI-02-validacao.md).
...
- [ ] `BNT-AI-02 — Chat e histórico individual`: **ativado no DEV; homologação de Felipe pendente**. `/assistente`, histórico privado, fases, cancelamento e fontes disponíveis. Login/inferência reais comprovados após publicação; resposta genérica de vendas identificada e corrigida em `0de3b139`, com 85 testes direcionados e validate aprovados. Smoke semântico publicado de sete/trinta dias aprovado; não encerrar pelo retorno `sem_dados` isoladamente. Testes de propriedade/RLS, dedupe, concorrência, revogação, timeout e saída tardia anteriores preservados. [Evidências e limites](evidencias/BNT-AI-02-validacao.md).
- [x] **Publicação do código AI-02 em DEV:** candidato `32a1685d` enviado somente a `origin/dev`; ação Easypanel `cmttdbkyj000c07o92hkbf4qx` terminou `done`; rollout `completed`, Codex `0.153.4` e arquivos da imagem conferidos. Página autenticada HTTP 200; API do Assistente HTTP 403 `ambiente_bloqueado`, conforme configurações ausentes. Não equivale a chat liberado nem aceite visual.
- [x] **Ativação técnica DEV da AI-02:** somente `local/vortek-erp-dev`, pela interface/API oficial, destino Supabase `.162`, piloto Felipe e perfil persistente exclusivo com login oficial. Inferência/histórico comprovados. Titular autorizou explicitamente liberar o piloto com dados; controles de treinamento não foram verificados independentemente e não são declarados confirmados. Precisão da resposta e aceite do usuário permanecem na AI-02/AI-GATE.
...
- Os casos LIVE ignorados não constituem aceite operacional: dependem de declaração/conta real e permanecem no marco correspondente. Naquele snapshot, a guarda comercial ainda estava restrita a `ML_PRICING_EXECUTION_MODE=test_only`, conta `test_user`, seller allowlisted e ambiente DEV/local.

**Próxima ação:** concluída tecnicamente por `BNT-REL-WRITER-01`; ainda são necessários `BNT-PARITY-FINAL`, `DELTA_PROMOCAO`, ensaio de restauração/migração e aceite operacional antes de fixar o SHA candidato.

### BNT-REL-WRITER-01 — Writers produtivos controlados — 09/09/2026

**Situação:** preparação técnica concluída e validada somente em `dev`; capacidade não ativada.

- O executor canônico passou a reconhecer `disabled`, `test_only` e `production_controlled`. Produção exige simultaneamente runtime `production`, origem `https://app.bentevi.shop`, Supabase resolvido exclusivamente para `.162`, seller allowlisted e `/users/me` exato, `MLB` e não `test_user`.
- A mesma pessoa `admin` ou `gerente` continua aprovando e aplicando. A interface exige modal final adicional em produção e exibe produto, SKU e preços; fechar o modal não chama a execução.
- Revalidação, claim SQL único, uma tentativa mutante, auditoria, captura imediata do ID criado, read-back e recuperação sem reenvio foram preservados. Os writers legados continuam usando o bloqueio incondicional.
...
- [x] ciclo completo de um pedido real do marco 7 comprovado por leitura no Bentevi e no Mercado Livre; [evidência](evidencias/BNT-MARCO-7-OPERACAO-01-validacao.md);
- [x] versão ativa e executor único atual do marco 7 comprovados em 17/09/2026; [evidência](evidencias/BNT-MARCO-7-OPERACAO-01-validacao.md);
- [x] acompanhamento diário dos primeiros sete dias do marco 7 aceito por confirmação expressa do responsável em 17/09/2026, incluindo 12/09; registros diários completos não foram localizados e a limitação está documentada na [evidência](evidencias/BNT-MARCO-7-OPERACAO-01-validacao.md);
- [x] release documental `e2626d97482410f8b19b2eeafaa7e38fb95f7322` registrada, com ação Easypanel concluída, imagem ativa e smoke; [evidência](evidencias/BNT-MARCO-7-OPERACAO-01-validacao.md);
...
**Ajuste produtivo BNT-CFG-07-PROD-01 (10/09/2026):** a política histórica de testes exclusivamente em homologação foi substituída depois da entrada do Bentevi em produção. A causa confirmada era tripla: o teste bloqueava `https://api.dslite.com.br`, enviava sempre `TipoAmbiente: 2` à Brasil NFe e tratava a resposta válida sem notas como falha. Probes produtivos prévios e somente de leitura confirmaram HTTP 200 com categorias válidas na DSLite e HTTP 200 com lista vazia e a mensagem conhecida de ausência de notas na Brasil NFe; nenhum pedido, compra, job ou documento fiscal foi criado.

A implementação passa a selecionar produção ou homologação pelo runtime, exige correspondência da URL DSLite e do ambiente fiscal, mantém allowlist/SSRF e usa somente os endpoints de consulta. O contrato administrativo acrescenta o ambiente do teste à resposta, a interface deixa de exibir homologação de forma fixa e o estado validado permanece retestável; salvar URL ou credencial continua invalidando a verificação. Não há migration nem alteração de credenciais. Foram aprovados 81 testes direcionados, `npm run validate` e `npm run build` com Next.js 16.3.3 e 127 páginas/rotas. Publicação e smoke produtivos são registrados no fechamento operacional desta implementação, sem autorizar a criação do primeiro pedido real.
...
## Fechamento M2M-PRC-03 — Consumidores legados (06/09/2026)

Primeira parcela e continuação concluídas. [Validação PRC-03](evidencias/M2M-PRC-03-validacao.md): consumidores migrados, 291 testes, validate e build aprovados, migration aplicada somente no `.162` e homologação autenticada de telas, simulador e PDFs concluída em `dev.bentevi.shop`.

**Estado: CONCLUÍDO no escopo de transição aprovado. Próxima ação: planejar BNT-CANON-QTY-01.** Isso não libera escrita comercial nem promoção. Os itens iniciais abaixo registram a primeira parcela; os seguintes, a conclusão da migração.
...
- [x] Migrar Produtos/detalhes, Anúncios, Catálogo, schemas, PDFs e simuladores ao DTO canônico; retirar fórmulas no browser.
- [x] Retirar fórmulas legadas das RPCs de busca/resumo, mantendo filtros, ordenação e paginação coerentes.
- [x] Migrar aritmética/tributação de pedidos sem inventar CMV histórico nem sobrescrever valores realizados.
- [x] Migrar formulário/contrato/RPC comercial; criar migration nova, ensaiar com rollback no `.162`, aplicar e regenerar tipos afetados.
- [x] Concluir inventário e retirada/bloqueio dos helpers/scripts restantes; comprovar ausência de consumidores decisórios legados habilitados.
...
### Fechamento BNT-PRICING-V2-04 — 07/09/2026

**Estado: implementação e validação local/SQL DEV concluídas; homologação externa pendente. Próxima ação: planejar BNT-PRICING-V2-05 — Override explícito.**

- [x] Fotografar `dev` limpa em `5e28343`, identificar produtores de preço e reutilizar economia/grupos existentes.
- [x] Separar avaliação, operação idempotente e evento imutável, com fontes explícitas e sem atribuição histórica fictícia.
- [x] Tornar captura e projeção atômicas; deduplicar observações entre tabelas e rejeitar preço antigo/conflitante.
- [x] Exigir baseline dos membros, versão do grupo e prova de resultado; manter operação incerta sem tentativa cega.
- [x] Acrescentar ID de avaliação e GET autenticado/paginado de histórico, sem redesenho de tela.
- [x] Aplicar migrations `20260907180000` e `20260907183000` somente no Supabase `.162`, após ensaios com rollback; gerar tipos nesse banco.
...
### Fechamento BNT-PRICING-V2-06 — 07/09/2026

- [x] Autorização de liquidação interna tipada, idempotente, auditada e gerida por admin/gerente;
- [x] Quantidade recortada no estoque interno existente; reposições não herdam exceção; kits usam componentes;
- [x] Limite de perda unitária explícito; economia canônica preservada e inconclusivo visível;
