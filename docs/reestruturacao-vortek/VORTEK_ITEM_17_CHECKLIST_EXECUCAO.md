...
- **Saneamento massivo de identidade — contenção P0, dry-run e execução produtiva parcial em 14–15/09/2026:** o guard e as migrations P0 foram aplicados no Supabase produtivo `.162`, e a release isolada `b28a9674` foi publicada sem delta BVF. O run produtivo final processou os 1.550 anúncios com Rodrigo como ator, liberou os 155 casos da auditoria executiva e manteve zero alteração em preço, vínculo, `produtos.ativo`, `custom_price`, estoque ou anúncio ML. Sete drifts novos da população original foram bloqueados; o estado real ficou em 1.519 `SEM_CONFLITO`, 22 `CONFLITO_CONFIRMADO` e 9 `PENDENCIA_VALIDACAO`. O safety stop `RECONCILIATION_FAILED:1550:1519:31:7` preservou a trilha e impede marcar a missão como concluída até reauditoria desses sete casos. [Evidências, gates e recuperação](evidencias/BNT-ML-CATALOG-IDENTITY-01-validacao.md).
