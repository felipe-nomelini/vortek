...
**Situação vigente (08/09/2026):** marco 2 encerrado no recorte inicial de Configurações. AI-00/01 concluídas no piloto individual; **código da BNT-AI-02 publicado em DEV**, candidato `32a1685d`, Codex `0.153.4` confirmado na imagem Railpack. [Evidências AI-02](evidencias/BNT-AI-02-validacao.md). **Próxima ação: ativar o Assistente no serviço DEV**, não avançar ao AI-GATE ainda. Faltam configuração administrativa no Easypanel, perfil persistente exclusivo, login oficial e confirmação dos controles da conta. O smoke autenticado confirma página publicada, mas o Assistente bloqueado por configuração; ainda não está pronto para teste conversacional. Assinatura ChatGPT e `gpt-6-astra/low` preservados. Liberação do sócio segue separada. PUB-GATE externo/ME2 no marco 6 e capacidade produtiva no marco 5. Nenhuma promoção ou execução comercial liberada.
...
**Prioridade vigente — atualização de 08/09/2026:** concluir ativação DEV da **`BNT-AI-02 — Interface e histórico`**. Push/deploy autorizados concluídos (`32a1685d`); 162 testes e validate/build passaram. Página publicada e Codex confirmado; o guard continua negando acesso porque as variáveis/perfil do serviço não foram configurados. [Pendências exatas](evidencias/BNT-AI-02-validacao.md#pendencias-para-o-teste-de-felipe). Necessário acesso administrativo válido ao Easypanel, login oficial exclusivo e confirmação dos controles da conta. Dados operacionais não foram enviados à IA. Modalidade e isolamento do sócio continuam gates separados.
...
**Decisão posterior de 08/09/2026:** primeiro teste **somente para Felipe**; acesso do sócio será uma liberação separada. AI-00/01 concluídas e código AI-02 publicado em DEV (`32a1685d`), ainda bloqueado por configuração e sem inferência real do chat. Não liberado para uso compartilhado ou produtivo. Manter somente assinatura ChatGPT, `gpt-6-astra/low`, sem API paga ou substituição automática. Concluir ativação e smoke conversacional DEV antes do AI-GATE. [Contrato canônico](VORTEK_BENTEVI_ASSISTENTE_CONTRATO.md) e [validação AI-02](evidencias/BNT-AI-02-validacao.md).
...
- [ ] `BNT-AI-02 — Chat e histórico individual`: **código publicado em DEV; ativação e homologação conversacional pendentes**. `/assistente`, menu após Dashboard condicionado ao piloto habilitado, histórico privado até exclusão, busca/renomeação/exclusão, fases de progresso, cancelamento e Drawer de fontes. Revalidados 162 testes, 2 LIVE ignorados, validate/build; SQL com rollback e migration no `.162` na implementação anterior; HTTP autenticado e Chromium local com fontes sintéticas aprovados. Smoke publicado confirma configuração ausente, não disponibilidade do chat. Outros usuários/admins negados, propriedade/RLS, dedupe, concorrência, revogação, timeout e saída tardia cobertos. [Evidências e limites](evidencias/BNT-AI-02-validacao.md). Não solicitar aceite visual antes da ativação/smoke abaixo; não houve inferência real do chat nesta entrega.
...
- [ ] **Ativação DEV da AI-02:** cumprir os [requisitos do contrato](VORTEK_BENTEVI_ASSISTENTE_CONTRATO.md#hospedagem-do-piloto--requisitos-para-a-publicação-da-ai-02): configurar somente `local/vortek-erp-dev` pela interface/API oficial do Easypanel, destino Supabase explícito `.162`, piloto, perfil persistente exclusivo e login oficial do titular. Acesso administrativo atual pendente; webhook de deploy disponível não substitui esse acesso. Manter dados bloqueados até confirmar controles da conta. Após configuração/deploy, executar smoke conversacional e fornecer roteiro a Felipe; não contornar guards.
