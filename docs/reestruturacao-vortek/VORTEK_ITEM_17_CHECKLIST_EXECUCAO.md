...
**Validação da revisão:** 28 cenários direcionados de cockpit, retornos, permissões e pricing aprovados; `npm run validate`, `npm run build` com 122 páginas e `git diff --check` aprovados. A função de reserva foi ensaiada com `ROLLBACK` antes da sincronização final e confirmou idempotência protegida pelo mesmo lock transacional do pedido. Nenhuma emissão, pré-visualização, consulta, cancelamento, CC-e ou e-mail foi enviado à Brasil NFe durante a validação.
...
**Rollback:** reverter `e9a1c83` e `59e0e11` em `dev`, redeployar somente `vortek-erp-dev` e, mediante migration corretiva exclusivamente no `.162`, remover a função, o vínculo de auditoria e a tabela de retornos enquanto ela permanecer vazia. Não há rollback ou ação em produção.
