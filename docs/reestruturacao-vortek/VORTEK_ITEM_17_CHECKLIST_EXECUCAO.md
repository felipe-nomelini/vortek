...
**Correção do pedido de pausa em `2026-09-17`:** a rota de status enviava `desired_price=570,68` junto com a pausa, apesar de `apply_price=false`. O gatilho de identidade do banco verifica qualquer preço não nulo; a função de leitura para `MLB7601891176` confirmou `ml_identity_price_write_blocked`, e nenhuma operação de pausa havia sido criada. A rota passou a enviar somente o status, sem preço ou quantidade; a notificação distingue falha de registro de falha após o envio. O SHA `5c62605b5f272bcc53f7949946dce998a824552e` passou em 41 testes direcionados, `npm run validate`, build, verificação de secrets e `git diff --check`; foi promovido para `bentevi-prod` e publicado pela ação Easypanel `cmu5z8a1z009z07ovaiq4e3in`, concluída. O serviço ativo apontou para esse SHA; saúde/login `200`, Anúncios `307`, APIs sem sessão `401`. O produto continuava ativo, com preço `570,68` e estoque `48`, e nenhuma pausa foi criada pelo deploy. **Pendente:** um único clique autenticado no anúncio, seguido do read-back da fila e do estado no Mercado Livre; sem essa prova não há aceite funcional da pausa.
