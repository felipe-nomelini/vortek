...
- **Vínculo de catálogo no preço manual — correção produtiva em 14/09/2026:** a revisão de preço passou a aceitar o vínculo canônico consistente de `catalogo_ml_snapshot`, mesmo quando a linha derivada ainda não existe em `anuncios_ml`; proprietário conflitante continua bloqueado. A sincronização observada deixou de enviar a coluna inexistente `listing_type_id` ao snapshot e agora explicita essa falha fatal. O caso real `VTK017371` foi reconciliado sem alterar preço nem enfileirar publicação. [Evidências e recuperação](evidencias/BNT-CATALOG-PRICE-LINK-01-validacao.md).
