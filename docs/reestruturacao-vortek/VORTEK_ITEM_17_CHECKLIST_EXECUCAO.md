...
- **Saneamento massivo de identidade — contenção P0 e dry-run integral em 14–15/09/2026:** a migration `20260914180000_bnt_ml_catalog_identity_01` foi ensaiada com rollback, aplicada e registrada no Supabase produtivo `.162`. O guard final bloqueia escrita automática de preço sem identidade `SEM_CONFLITO` e fonte ML atual, preserva preço manual individual e invalida a liberação quando o snapshot material muda. A fonte canônica reconstruída do PDF passou SHA-256, 1.550 linhas/MLBs e 1.549 SKUs; o dry-run server-side classificou individualmente 21 conflitos confirmados, 158 pendências e 1.371 identidades coerentes, com uma anomalia `listed`, zero erro e sem deduplicar os dois anúncios de `VTK009697`. Foram somente 3.308 GETs e 8 HEADs; readback e ledger confirmaram zero alteração em vínculo, preço, `produtos.ativo`, `custom_price`, estoque ou anúncio ML. Os dez artefatos e o manifesto foram gerados; correções e filas econômicas permanecem bloqueadas até aprovação explícita. [Evidências, gates e recuperação](evidencias/BNT-ML-CATALOG-IDENTITY-01-validacao.md).
