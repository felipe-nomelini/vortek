...
**V2-15 operacional — 08/09/2026:** recorte publicado em DEV (`982be7b2`): três parâmetros auditáveis, faixas fixas retornadas pela API, contexto fiscal e simulação separada entre salvo/formulário. Revalidados 100 testes Node (incluindo contrato de deploy), 13 checkpoints de navegador isolado e validate/build. Easypanel `done`, versão em execução e smoke remoto sem sessão confirmados. [Evidências e limites](evidencias/BNT-PRICING-V2-15-operacional-validacao.md). Próximo passo: usuário conferir Configurações → Comercial e conceder aceite inicial D20. Não fecha marco 2 nem V2-15 integral automaticamente.
