...
A migration `20260905023000_bnt_cfg_06_notifications.sql` consolidou os destinatários legados, criou política e alvos Push normalizados, adicionou constraints, RLS, grants mínimos e RPC atômico de salvamento. Ela foi ensaiada e aplicada apenas no `supabase-dev` em `192.168.1.162`. Testes de canal continuam ações explícitas: Push alcança somente o navegador atual, WhatsApp somente o número de teste do ambiente e SMTP executa verificação de conexão sem enviar e-mail. A aprovação visual em homologação é a única pendência antes de `BNT-CFG-07`.
