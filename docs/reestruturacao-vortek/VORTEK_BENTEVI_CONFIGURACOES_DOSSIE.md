...
**V2-15 operacional — 08/09/2026:** recorte publicado em DEV (`982be7b2`), com [validações e aceite inicial D20 concedido pelo usuário](evidencias/BNT-PRICING-V2-15-operacional-validacao.md#aceite-inicial-do-usuario--08092026). Marco 2 encerrado no recorte inicial; V2-15/D20 integrais permanecem abertos. AI-00 [concluída para o piloto individual](evidencias/BNT-AI-00-validacao.md#fechamento-individual); próxima ação: planejar AI-01. Somente assinatura ChatGPT e `gpt-6-astra/low`, sem nova alteração de Configurações.
...
Validação local: `npm run test:bentevi-integrations` (72 testes), `npm run validate` e build passaram. Um teste histórico Hayamax foi atualizado para não confundir a função aposentada com `getMercadoPagoPaymentForMlSale`, já existente no HEAD antes da entrega. Commit `77781c5` publicado somente em DEV, com ação Easypanel concluída, SHA confirmado, bundle público e smoke HTTP verificados. Renderização local com dados sintéticos a 1440/390 px sem transbordamento. Testes autenticados de escrita/provedores remotos não foram executados. Aprovação visual permanece pendente; não avançar automaticamente ao pricing.
...
**Ajuste produtivo BNT-CFG-07-PROD-01 (10/09/2026):** após a virada do Bentevi, a política acima restrita a homologação deixou de representar o runtime ativo. O teste agora deriva o ambiente de `VORTEK_RUNTIME_ENVIRONMENT`: produção aceita somente a origem oficial `https://api.dslite.com.br` e consulta categorias por GET; o DEV local preserva `https://api.master.dev.dslite.com.br`. Na Brasil NFe, produção exige `BRASILNFE_TIPO_AMBIENTE=1` e consulta `ObterNotasFiscais` com `TipoAmbiente: 1`; homologação continua usando `2`. Inconsistências entre runtime, URL e ambiente fiscal falham fechadas, sem alterar cadastro ou credencial.

A resposta produtiva observada da Brasil NFe para uma consulta válida sem correspondência foi HTTP 200, `Notas: []` e `Error: "Não existe notas fiscais para o período informado."`. Somente essa condição conhecida, normalizada e acompanhada da lista vazia é aceita como conectividade válida; demais erros continuam recusados. A interface identifica dinamicamente produção ou homologação, e uma configuração validada pode ser testada novamente. Os probes permanecem consultas: não emitem, importam, devolvem ou cancelam documentos e não criam pedidos DSLite. Nenhuma migration ou alteração de credenciais integra este ajuste.

### Continuidade da política de pricing

**Padronização dos cabeçalhos (05/09/2026):** cards aprovados pelo responsável, com pedido adicional de uniformidade tipográfica. As oito abas consomem o mesmo `ConfiguracoesTabHeading` (somente título/descrição, sem sobrescritas de estilo por aba): 20 px, peso 600, linha 28 px; descrição 14/22 px; separação 6 px; cores de `benteviColors`. Usuários e Histórico receberam cabeçalhos equivalentes, mantendo contagem e filtros. Estilos locais conflitantes removidos, sem alterar títulos internos ou funcionalidades. 76 testes, validate e build passaram; tipografia computada idêntica em galeria com os oito cabeçalhos reais e CSS compilado a 1440/390 px. Evidência de publicação na seção BNT-CFG-07 do checklist.
