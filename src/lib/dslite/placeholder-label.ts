...
const EVOLUSOM_PLACEHOLDER_LABEL_PATH = path.join(
  process.cwd(),
  'public',
  'dslite',
  'labels',
...
export function getDslitePlaceholderLabelConfig(
  ...
  if (isEvolusomSupplier(fornecedorId, fornecedorNome)) {
    return {
      ...
      fileName: DSLITE_EVOLUSOM_PLACEHOLDER_LABEL_FILE_NAME,
      path: EVOLUSOM_PLACEHOLDER_LABEL_PATH,
      supplierLabel: 'Evolusom',
    };
  }
