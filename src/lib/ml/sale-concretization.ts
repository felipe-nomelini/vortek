...
export function assessMlSaleConcretization(input: {
  ...
  if (normalized(input.orderStatus) !== 'paid') {
    return { concretized: false, reason: 'order_not_paid' };
  }
  if (
    normalized(input.shipmentStatus) !== 'shipped'
    || normalized(input.shipmentSubstatus) !== 'stale'
  ) {
  ...
  if (input.payments.some((payment) => normalized(payment.status) !== 'approved'
    || normalized(payment.money_release_status) !== 'released')) {
