...
export default function CommercialPolicyControls({productId,itemId}:{productId:string;itemId?:string|null}) {
  ...
  const [duration,setDuration]=useState<number|null>(null),[unit,setUnit]=useState('meses');
  ...
  function saveWarranty() {
    ...
    const evidence=[{...subject,origin,duration,unit,offerId:origin==='GARANTIA_FORNECEDOR'?identity?.warrantyOfferId:null}];
    return write(`/api/produtos/${productId}`,'PATCH',{reason,warrantyEvidence:{evidence}});
  }
  return <Card title="Políticas comerciais" style={{marginBottom:16}}><Space direction="vertical" style={{width:'100%'}}>
    ...
    <Typography.Text>{warranty?.resolution?.status==='resolved'?`${warranty.resolution.origin}: ${warranty.resolution.duration} ${warranty.resolution.unit}. Fonte: ${warranty.resolution.source}`:'Pendente: revisar evidências de garantia contraditórias ou indisponíveis.'}</Typography.Text>
    <Select value={origin} onChange={setOrigin} options={[{value:'FABRICANTE',label:'Fabricante comprovado'},{value:'GARANTIA_FORNECEDOR',label:'Fornecedor comprovado'}]}/>
    <Typography.Text>Sem prazo comprovado de fabricante ou fornecedor, aplica-se automaticamente garantia do vendedor de 30 dias.</Typography.Text>
    <Space><InputNumber min={1} precision={0} value={duration} onChange={setDuration} placeholder="Prazo comprovado"/><Select value={unit} onChange={setUnit} options={['dias','meses','anos'].map(v=>({value:v,label:v}))}/></Space>
    <Input value={source} onChange={e=>setSource(e.target.value)} placeholder="Fonte / documento e evidência específica deste produto"/>
    <Button disabled={busy||!reason.trim()||!source.trim()||!duration} onClick={saveWarranty}>Registrar evidência revisada</Button>
  </Space></Card>;
}
