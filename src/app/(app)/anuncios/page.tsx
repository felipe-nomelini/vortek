...
import ManualMlPriceButton from '@/components/products/ManualMlPriceButton';

import { CompetitivePricingSummary, ListingValidationNotice, PricingQuoteSummary } from '@/components/products/LivePricingQuote';
...
type PricingDetails = {
  ...
  listingValidation?: PricingListingValidation | null;
  currentPrice: number;
...
const EMPTY_METRICS: MlListingMetrics = { total: 0, active: 0, paused: 0, sold: 0, visitedUnsold: 0, qualityRisk: 0, priceReview: 0 };
...
export default function AnunciosPage() {
  ...
  const queueOptions = useMemo(() => ([
    ...
    { value: 'visited_unsold', label: <span className={styles.quickViewLabel}>Com visitas, sem vendas <strong className={styles.quickViewCount}>{queueCounts.visitedUnsold}</strong></span> },
  ...
  const qualityMissingAttributes = Array.isArray((activeAnalysis?.qualityInfo as any)?.missing_attributes)
    ...
    : Array.isArray((activeAnalysis?.qualityInfo as any)?.catalog_quality?.missing_attributes)
      ? (activeAnalysis?.qualityInfo as any).catalog_quality.missing_attributes as string[]
      : [];
  ...
  return <div className={styles.page}>
    ...
    <Segmented className={styles.quickViews} options={queueOptions} value={focus} onChange={(value) => {
      const nextFocus = value as MlListingsFocus;
      setFocus(nextFocus);
      if (nextFocus === 'visited_unsold') setSort({ sortBy: 'visits', sortOrder: 'desc' });
    }} />
    ...
    <section className={styles.filterBar}>
      ...
      <Space.Compact className={styles.priceFilter}><InputNumber {...currencyInputProps} value={priceMin} onChange={(value) => setPriceMin(value ?? null)} min={0} prefix="R$" placeholder="Preço mín." /><InputNumber {...currencyInputProps} value={priceMax} onChange={(value) => setPriceMax(value ?? null)} min={0} prefix="R$" placeholder="Preço máx." /></Space.Compact>
      ...
      {activeAnalysis && <div className={styles.drawerContent}>
        ...
        <Spin spinning={detailsLoading}>
          ...
          <section className={styles.drawerSection}><div className={styles.sectionHeading}><div><span>Preço e rentabilidade</span><strong>Um preço para os anúncios vinculados</strong></div></div>
            ...
            <div className={styles.priceEditor}><div><label>Novo preço de venda</label><InputNumber {...currencyInputProps} value={newPrice} onChange={(value) => setNewPrice(value ?? null)} min={0.01} precision={2} prefix="R$" disabled={!details || Boolean(visualReview)} /></div><div><label>Novo lucro unitário</label><strong className={(nextProfit || 0) >= 0 ? styles.positive : styles.negative}>{nextProfit === null ? '—' : formatCurrency(nextProfit)}</strong></div>{details?.catalog?.priceToWin && <Button onClick={() => void simulateCompetitivePrice()}>Simular referência competitiva</Button>}<ManualMlPriceButton productId={activeAnalysis.productId!} itemId={activeAnalysis.itemId} priceCents={newPrice == null ? undefined : Math.round(newPrice * 100)} disableAutomaticPricing={details?.automaticPricing?.active === true} disabled={Boolean(visualReview) || !activeAnalysis.productId} /></div>
