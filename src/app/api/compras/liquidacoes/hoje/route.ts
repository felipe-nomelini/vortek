...
export async function GET(request: Request) {
  ...
  const purchases: Array<{ id: string; dsid: string | null; evolusom_order_id: number | null;
    pedido_id: string | null; data_criacao: string; fornecedor_id: string | null;
  ...
  const sales = new Map<string, Array<{ id: string; numero: number; dslite_id: string | null;
    evolusom_order_id: number | null; situacao: string | null;
  ...
  for (let index = 0; index < visible.length; index += 100) {
    ...
    const [saleResult, directSaleResult, allocationResult, divergenceResult] = await Promise.all([
    ...
    if (saleResult.error || directSaleResult.error || allocationResult.error || divergenceResult.error) return NextResponse.json({ error: 'Falha ao consultar elegibilidade' }, { status: 500 });
    for (const sale of saleResult.data || []) {
      if (!canUseHomologationFixtures() && isHomologationFixtureSource(sale.snapshot_source)) continue;
      const key = `dslite:${sale.dslite_id}`;
      sales.set(key, [...(sales.get(key) || []), sale]);
    }
    for (const sale of directSaleResult.data || []) {
      if (!canUseHomologationFixtures() && isHomologationFixtureSource(sale.snapshot_source)) continue;
      sales.set(`evolusom:${sale.id}`, [sale]);
    }
  ...
  for (const supplier of suppliers || []) {
    ...
    const rows = own.map((purchase) => {
      ...
      const linked = direct
        ? (sales.get(`evolusom:${purchase.pedido_id}`) || [])
          .filter((sale) => sale.evolusom_order_id === purchase.evolusom_order_id)
        : (sales.get(`dslite:${purchase.dsid}`) || []);
      ...
      return { compraId: purchase.id, dsid: String(direct ? purchase.evolusom_order_id : purchase.dsid || ''),
        source: direct ? 'evolusom' : 'dslite', dataCriacao: purchase.data_criacao,
  ...
  const unassigned = visible.filter((row) => !knownSupplierIds.has(String(row.fornecedor_id || '')))
    .map((row) => ({ compraId: row.id, dsid: String(row.evolusom_order_id ?? row.dsid ?? ''),
      source: row.evolusom_order_id != null ? 'evolusom' : 'dslite',
