...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const parsed = z.object({ decisao: z.enum(['sent', 'not_sent']), justificativa: z.string().trim().min(10).max(500) })
    .strict().safeParse(await request.json().catch(() => null));
  if (!z.string().uuid().safeParse(id).success || !parsed.success) {
    return NextResponse.json({ error: 'Resolução inválida' }, { status: 422 });
  }
  const { data, error } = await createServiceClient().rpc('supplier_oracle_resolve_communication', {
    p_id: id, p_decision: parsed.data.decisao, p_note: parsed.data.justificativa, p_actor: auth.userId,
  });
