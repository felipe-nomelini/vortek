...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const parsed = z.object({ versaoEsperada: z.number().int().positive() }).strict()
    .safeParse(await request.json().catch(() => null));
  if (!z.string().uuid().safeParse(id).success || !parsed.success) {
    return NextResponse.json({ error: 'Aprovação inválida' }, { status: 422 });
  }
  const { data, error } = await createServiceClient().rpc('supplier_oracle_communication_approve', {
    p_id: id, p_expected_version: parsed.data.versaoEsperada, p_actor: auth.userId,
  });
