...
function looksLikeDescriptionTitle(input: unknown): boolean {
  const text = normalizeText(input);
  if (text.length > 120) return true;
  return /\b(?:é um|é uma|oferecendo|proporciona|garantem|ideal para|possui|acompanha)\b/i.test(
    text,
  );
}

function toDisplayProductName(input: string): string {
  return input
    .split(/\s+/)
    .filter(Boolean)
    .map((word) => {
      if (/^\d+(?:[.,]\d+)?[A-Z]+$/i.test(word)) return word.toUpperCase();
      if (/^[A-Z]{2,}\d*$/i.test(word) && word.length <= 6)
...
function beautifyShortProductName(input: unknown): string {
  let text = normalizeText(input);
  if (!text) return "";

  text = text
    .replace(/\bF\s+SOLDA\b/i, "Ferro De Solda")
    .replace(/\bC\/\b/gi, "Com")
    .replace(/\bP\/\b/gi, "Para");

  const solderMatch =
    /^Ferro De Solda\s+(\d+W)\s+([A-Z0-9]+)\s+([A-Z0-9]+)\s+(\d+V)$/i.exec(
      text,
    );
  if (solderMatch) {
    const [, power, brand, model, voltage] = solderMatch;
    return `Ferro De Solda ${toDisplayProductName(brand)} ${toDisplayProductName(model)} ${power.toUpperCase()} ${voltage.toUpperCase()}`;
  }

  return toDisplayProductName(text);
}

function pickBestProductName(item: any, sku: string): string {
  const titulo = normalizeText(item?.titulo);
  const tituloCurto = normalizeText(item?.titulo_curto);

  if (tituloCurto && looksLikeDescriptionTitle(titulo)) {
    return fallbackNome(beautifyShortProductName(tituloCurto), sku);
  }

  return fallbackNome(titulo, sku);
}
...
function isProductSkuUniqueViolation(error: any): boolean {
  const text =
    `${error?.message || ""} ${(error as any)?.details || ""}`.toLowerCase();
  return (
    text.includes("produtos_sku_key") ||
    text.includes("produtos_sku_upper_unique")
  );
}
...
async function insertProductRowsWithGeneratedSkus(
  ...
  return {
    data: null,
    error: new Error(
      "Falha ao gerar SKU mestre único após múltiplas tentativas",
    ),
  };
}
...
export async function POST(req: Request) {
  ...
  const maxPagesPerRun = parsePositiveInt(
    body?.maxPagesPerRun,
  ...
  try {
    ...
    if (!fornecedores || fornecedores.length === 0) {
      errors.push({
        ...
        message: "Nenhum fornecedor retornado pela DSLite",
      });
      return NextResponse.json(
        {
          success: false,
    ...
    const { data: fornecedoresAtivosLocal, error: fornecedoresAtivosError } =
      await client
        .from("fornecedores")
        .select("dslite_id")
        .eq("ativo", true)
    ...
    if (fornecedoresAtivosError) {
      throw new Error(
        `Falha ao consultar fornecedores ativos locais: ${fornecedoresAtivosError.message}`,
      );
    }
    ...
    const fornecedorIdsRaw: Array<string | number> = Array.isArray(
      body?.fornecedorIds,
    )
      ? body.fornecedorIds
      : [];
    ...
    for (const fornecedor of fornecedores) {
      fornecedorMap.set(
        ...
        String(fornecedor.apelido || fornecedor.id),
      );
    }
    ...
    if (fornecedorIdsAtivos.length === 0) {
      errors.push({
        code: "dslite_fornecedor_ids_empty",
        message: "Nenhum fornecedor ativo selecionado para catálogo",
      });
      return NextResponse.json(
        {
          success: false,
    ...
    const startSupplierIndex = cursorFornecedorId
      ? fornecedorIdsAtivos.indexOf(cursorFornecedorId)
      : 0;
    ...
    while (supplierIndex < fornecedorIdsAtivos.length) {
      ...
      const fornecedorNome =
        fornecedorMap.get(Number(fornecedorId)) || fornecedorId;

      while (remainingPagesBudget > 0) {
        const response = await sincronizarCatalogo(
          fornecedorId,
          page,
          pageSize,
        );
        ...
        const batch = produtos.map((item) => {
          const supplierSku = buildCanonicalDsliteSku(
            fornecedorId,
            item.produtoid_empresa,
            item.produtoid,
          );
          const normalizedGtin = normalizeGtin(item.ean11);
          return {
            sku_fornecedor: supplierSku,
            nome: pickBestProductName(item, supplierSku),
            marca: item.marca || "",
        ...
        const dsliteProdutoIds = batch
          .map((row) => row.dslite_produto_id)
          .filter(Boolean);
        const gtins = Array.from(
          new Set(
            batch.map((row) => String(row.gtin || "").trim()).filter(Boolean),
          ),
        );
        const [existingOffersResp, existingProductsResp, gtinProductsResp] =
          await Promise.all([
            client
            ...
            client
              .from("produtos")
              .select(
                "id,sku,nome,marca,gtin,descricao,imagens,dslite_fornecedor_id,dslite_produto_id,ativo",
              )
              .eq("dslite_fornecedor_id", String(fornecedorId))
              .in("dslite_produto_id", dsliteProdutoIds),
            gtins.length > 0
        ...
        if (
          existingOffersResp.error ||
          existingProductsResp.error ||
          gtinProductsResp.error
        ) {
          errors.push({
            code: "catalog_existing_select_failed",
            message:
              existingOffersResp.error?.message ||
              existingProductsResp.error?.message ||
              gtinProductsResp.error?.message ||
              "Falha ao consultar produtos/ofertas existentes",
        ...
        for (const row of batch) {
          ...
          const legacyProduct = existingProductByDsliteIdentity.get(
            identityKey,
          ) as any;
        ...
        if (productRowsToCreate.length > 0) {
          for (const payload of chunk(productRowsToCreate, UPSERT_CHUNK_SIZE)) {
            const { data: insertedRows, error: upsertError } =
              await insertProductRowsWithGeneratedSkus(client, payload);
            ...
            for (
              let index = 0;
              index < (insertedRows || []).length;
              index += 1
            ) {
              const inserted = (insertedRows || [])[index] as any;
              const productKey = String(
                (payload[index] as any)?._product_key || "",
              ).trim();
              if (productKey) {
                for (const offerRow of pendingOfferRows) {
                  if (
                    String((offerRow as any)._product_key || "") === productKey
                  ) {
        ...
        const offerPayload = pendingOfferRows
          ...
          .map((row) => {
            const { _product_key, ...payload } = row as Record<string, unknown>;
            return payload;
          });
        ...
        if (offerPayload.length > 0) {
          const { error: offerUpsertError } = await client
            .from("produto_fornecedor_ofertas")
            .upsert(offerPayload as any, {
              onConflict: "dslite_fornecedor_id,dslite_produto_id",
            });
        ...
        try {
          await syncPreferredProductSnapshot(
            client,
          ...
          errors.push({
            ...
            message:
              err?.message ||
              "Falha ao sincronizar fornecedor preferencial do produto",
            context: { fornecedorId, page },
          });
          break;
        }
        ...
        const totalRegistros = Number(
          response?.detalhesConsulta?.totalRegistros || 0,
        );
        const perPage = Number(
          response?.detalhesConsulta?.limit || produtos.length || pageSize,
        );
        const totalPaginas =
          perPage > 0 ? Math.ceil(totalRegistros / perPage) : page;
        ...
        if (remainingPagesBudget <= 0) {
          if (hasMore) {
            ...
            nextCursor = nextSupplierId
              ? { fornecedorId: String(nextSupplierId), page: 1 }
              : null;
          }
          stopByBudget = true;
          break;
    ...
    return NextResponse.json({
      ...
      errors,
      duration: { ms: Date.now() - startedAt },
      deprecations: withMlSync
        ? ["withMlSync foi ignorado nesta rota e será removido"]
        : [],
