...
async function resolvePackIdFromMl(orderId: string): Promise<string | null> {
  ...
  try {
    const detailAlt = await fetchML<any>(`/orders/${orderId}`, { headers: { 'x-format-new': 'true' } });
...
export async function POST(request: Request) {
  ...
  for (const pedido of pedidos || []) {
    try {
      ...
      await registrarEventoNfAuditoria({
        ...
        evento: 'pack_id_backfill',
        respostaMl: { motivo: 'erro_processamento', erro: err?.message || 'erro_desconhecido' },
        statusResultante: 'failed',
      });
    }
  }
  ...
  return NextResponse.json({
    ...
    return NextResponse.json({
      success: false,
      domain,
      errors: [{ code: 'pack_backfill_unexpected_error', message: err?.message || 'Erro inesperado no backfill de pack_id' }],
      records: { processed: 0, updated: 0, not_found: 0, failed: 0 },
      duration: { ms: Date.now() - startedAt },
