...
const querySchema = z.object({ before: z.string().regex(/^\d+$/).refine(value => Number.isSafeInteger(Number(value))).optional(), itemId: z.string().regex(/^ML[A-Z]\d+$/).optional(), groupId: z.string().uuid().optional(), limit: z.coerce.number().int().min(1).max(100).default(30) }).strict();
