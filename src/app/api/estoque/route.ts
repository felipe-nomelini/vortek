...
export async function GET(request: Request) {
  ...
  const [positionsResult, receiptsResult, receiptItemsResult, movementsResult, returnsResult] = await Promise.all([
    (db as any)
      .from('estoque_interno_posicoes_detalhadas')
      .select('produto_id,sku,nome,fisico_util,reservado,disponivel,em_revisao,nao_aproveitavel,devolucoes_disponivel,compras_disponivel,ajustes_disponivel,ultima_movimentacao_em')
    ...
    (db as any)
      ...
      .select('id,produto_id,pedido_id,tipo,quantidade,motivo,situacao_estoque,status_devolucao,estado_envio_interno,origem_estoque,created_at,despachado_em,estornada_em,estorno_motivo,recebimento_id,created_by,snapshot_source,produtos(sku,nome),pedidos(ml_order_id,ml_pack_id),estoque_recebimentos_nfe(chave_nfe,numero,serie,emitente_nome)')
      .order('created_at', { ascending: false })
      .limit(500),
    (db as any)
      .from('estoque_devolucoes_ml')
      .select('id,pedido_id,produto_id,quantidade,motivo,status_logistico,estado_operacional,recebido_em,decidido_em,observado_em,created_at,updated_at,produtos(sku,nome),pedidos(ml_order_id,ml_pack_id)')
      .order('updated_at', { ascending: false })
      .limit(500),
  ]);
  for (const result of [positionsResult, receiptsResult, receiptItemsResult, movementsResult, returnsResult]) {
  ...
  for (const movement of movementRows.filter((row: any) => row.snapshot_source === BNT_D05_INVENTORY_FIXTURE_SOURCE)) {
    ...
    const current = fixtureByProduct.get(productId) || {
      ...
      em_revisao: 0,
      nao_aproveitavel: 0,
      devolucoes_disponivel: 0,
      compras_disponivel: 0,
      ajustes_disponivel: 0,
      ultima_movimentacao_em: null,
      is_homologation_fixture: true,
    };
    const quantity = Number(movement.quantidade || 0);
    if (['entrada_devolucao', 'entrada_compra', 'ajuste_positivo'].includes(movement.tipo) && movement.situacao_estoque === 'liberado') {
      current.fisico_util += quantity;
      if (movement.tipo === 'entrada_devolucao') current.devolucoes_disponivel += quantity;
      else if (movement.tipo === 'entrada_compra') current.compras_disponivel += quantity;
      else current.ajustes_disponivel += quantity;
    }
  ...
  const returnsByState = (state: string) => returns
    .filter((row: any) => row.estado_operacional === state)
    .reduce((total: number, row: any) => total + Number(row.quantidade || 0), 0);
  ...
  return NextResponse.json({
    ...
    summary: {
      ...
      emConferencia: inConference + returnsByState('aguardando_inspecao'),
      devolucoesCaminho: returnsByState('em_transito'),
      devolucoesReceber: returnsByState('entrega_informada'),
      devolucoesInspecao: returnsByState('aguardando_inspecao'),
    },
...
export async function POST(request: Request) {
  ...
  const db = createServiceClient();
  const { data, error } = await (db as any).rpc('adjust_internal_stock_by_origin', {
