...
export async function GET(request: Request, props: { params: Promise<{ id: string }> }) {
  ...
  const { data: receipt, error } = await (db as any).from('estoque_recebimentos_nfe')
    .select('chave_nfe,numero,xml_nfe,snapshot_source,provider_status')
    .eq('id', (await props.params).id).maybeSingle();
  ...
  try {
    const content = receipt.xml_nfe
      ? Buffer.from(receipt.xml_nfe, 'utf-8')
      : await obterDocumentoEntradaBrasilNfe(receipt.chave_nfe, 1);
    if (!content) return NextResponse.json({ error: 'XML ainda não disponível.' }, { status: 404 });
    return new NextResponse(new Uint8Array(content), {
      headers: {
        'Content-Type': 'application/xml; charset=utf-8',
        'Content-Disposition': `attachment; filename="nfe-entrada-${receipt.numero || receipt.chave_nfe}.xml"`,
        'Cache-Control': 'private, no-store',
      },
    });
  } catch (lookupError: any) {
    return NextResponse.json({ error: lookupError?.message || 'Falha ao obter XML.' }, { status: 502 });
  }
}
