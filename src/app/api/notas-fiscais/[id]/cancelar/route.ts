...
const DEFAULT_JUSTIFICATIVA =
  "Cancelamento operacional da NF-e solicitada pelo usuário";
...
export async function POST(
  ...
  if (!cancel.ok) {
    ...
    return NextResponse.json(
      { error: cancel.error || "Falha ao cancelar nota fiscal" },
      { status: 502 },
    );
  }
  ...
  if (updateError) {
    await registrarEventoNfAuditoria({
      ...
      respostaMl: {
        error: "Falha ao persistir cancelamento local",
        db_error: updateError.message,
        provider_raw: cancel.raw || null,
      },
      statusResultante: "failed_local_persist",
    });
    return NextResponse.json(
      { error: "Falha ao atualizar status local da nota fiscal" },
      { status: 500 },
    );
  }
