...
import { Bkr1KitGtinTable } from "./Bkr1KitGtinTable";
...
export default async function KitsSemAnuncioPublicPage({ searchParams }: PageProps) {
  ...
  const rows: PublicRow[] = ((produtos || []) as ProdutoRow[])
    .filter((produto) =>
      kitIds.has(String(produto.id)) &&
      (!String(produto.gtin || "").trim() || SPECIAL_REQUIREMENTS[String(produto.sku || "")]),
    )
    .map((produto) => ({
  ...
  const expiresAt = searchParams?.expires || null;
  const canEdit = verifyPublicBkr1KitToken(token, expiresAt);

  return (
    <main style={{ maxWidth: 1180, margin: "0 auto", padding: "32px 20px 48px", color: "#f0f0f0" }}>
      <header style={{ marginBottom: 24 }}>
        <p style={{ color: "#8c8c8c", margin: 0 }}>Vortek · BKR1</p>
        <h1 style={{ margin: "8px 0", fontSize: 28, color: "#fff" }}>Kits pendentes de anúncio</h1>
        ...
        {canEdit && <p style={{ color: "#95de64", margin: "12px 0 0" }}>Você pode informar e salvar o GTIN de cada kit nesta página.</p>}
      </header>
      ...
      <Bkr1KitGtinTable rows={pageRows} token={canEdit ? token : null} expiresAt={canEdit ? expiresAt : null} />

      {totalPages > 1 && (
        <nav style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 12, marginTop: 20 }}>
          {page > 1 ? <Link href={`?pagina=${page - 1}${canEdit ? `&expires=${encodeURIComponent(expiresAt!)}&token=${encodeURIComponent(token!)}` : ""}`}>← Anterior</Link> : <span style={{ color: "#595959" }}>← Anterior</span>}
          <span style={{ color: "#bfbfbf" }}>Página {page} de {totalPages}</span>
          {page < totalPages ? <Link href={`?pagina=${page + 1}${canEdit ? `&expires=${encodeURIComponent(expiresAt!)}&token=${encodeURIComponent(token!)}` : ""}`}>Próxima →</Link> : <span style={{ color: "#595959" }}>Próxima →</span>}
        </nav>
      )}
    </main>
  );
