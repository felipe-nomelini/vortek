...
async function getMlManualBlockSet(
  ...
  const itemIds = Array.from(
    new Set(
      produtos.map((p) => String(p.ml_item_id || "").trim()).filter(Boolean),
    ),
  );
  ...
  if (itemIds.length > 0 && skus.length > 0) {
    query = query.or(
      `ml_item_id.in.(${itemIds.join(",")}),sku.in.(${skus.join(",")})`,
    );
...
function classifyMlItemError(
  ...
  const causeCodes = causes.map((c: any) =>
  ...
  if (status >= 500) {
    return {
      ...
      status,
      error: message,
      reason_code: "ml_server_error",
      transient: true,
    };
  }
  if (
    status === 400 &&
    (lowerMessage.includes("under_review") || lowerMessage.includes("moderat"))
  ) {
    return {
      success: false,
      status,
...
async function mlItemRequest<T = any>(
  path: string,
  options?: RequestInit,
...
async function withMlRetry<T>(
  fn: () => Promise<MlRequestResult<T>>,
): Promise<MlRequestResult<T> & { attempts: number }> {
  let attempt = 0;
...
export async function sincronizarEstoqueComML(
  ...
  const blockedReasonCodes: SyncReasonCode[] = [
    "under_review",
    "field_not_updatable",
  ];
  ...
  const blockStateByItemId = new Map<
    string,
    {
  ...
  if (mlItemIds.length > 0) {
    const { data: blockRows } = await serviceClient
      .from("anuncios_ml")
      .select(
        "ml_item_id, ml_sync_blocked_until, ml_sync_block_reason, ml_sync_last_error",
      )
  ...
  const setBlockedCooldown = async (
    mlItemId: string,
    reasonCode: SyncReasonCode,
    errorMessage?: string,
  ) => {
    const blockedUntil = new Date(
      Date.now() + ML_BLOCK_COOLDOWN_HOURS * 60 * 60 * 1000,
    ).toISOString();
    await serviceClient
      .from("anuncios_ml")
      .update({
    ...
    blockStateByItemId.set(mlItemId, {
      blockedUntil,
      reason: reasonCode,
  ...
  for (const produto of produtos) {
    ...
    const skuUpper = String(produto.sku || "")
      .trim()
      .toUpperCase();
    const manualReason =
      manualBlockSet.byMlItemId.get(produto.ml_item_id) ||
      (skuUpper ? manualBlockSet.bySku.get(skuUpper) : undefined);
    if (manualReason) {
      ...
      if (resultado.manual_block_items.length < 20) {
        resultado.manual_block_items.push({
          ml_item_id: produto.ml_item_id,
          sku: produto.sku,
          motivo: manualReason,
        });
    ...
    const blockedUntil = blockState?.blockedUntil
      ? new Date(blockState.blockedUntil)
      : null;
    ...
    if (isBlockedNow) {
      detalhe = {
        ...
        erro:
          ...
          `Item em cooldown de bloqueio ML até ${blockedUntil.toISOString()}`,
    ...
    if (estoque === 0) {
      ...
      let statusFinal =
        statusDepoisUpdate.data?.status || updateZero.data?.status;
      if (statusFinal !== "paused") {
        ...
        statusFinal =
          statusFinalCheck.data?.status ||
          forcePause.data?.status ||
          statusFinal;
      ...
      if (confirmadoPausado) {
        ...
        const blockedUntilIso = isBlockedByMlRule(fallbackReason)
          ? await setBlockedCooldown(
              produto.ml_item_id,
              fallbackReason,
              updateZero.error || statusDepoisUpdate.error || undefined,
            )
          : undefined;
        detalhe = {
          ...detalhe,
          ...
          erro:
            updateZero.error ||
            statusDepoisUpdate.error ||
            "Item não pausou após atualização de estoque zero",
      ...
      const updateBeforeReactivate = await atualizarQuantidadeItemML(
        produto.ml_item_id,
        estoque,
      );
      const statusBeforeReactivate = await obterStatusItemML(
        produto.ml_item_id,
      );
      const quantityInMl = Number(
        statusBeforeReactivate.data?.available_quantity ??
          updateBeforeReactivate.data?.available_quantity ??
          0,
      );
      let totalAttempts =
        updateBeforeReactivate.attempts + statusBeforeReactivate.attempts;

      if (
        !updateBeforeReactivate.success &&
        !updateBeforeReactivate.transient
      ) {
        const reasonCode =
          updateBeforeReactivate.reason_code ||
          statusBeforeReactivate.reason_code;
        const blockedUntilIso = isBlockedByMlRule(reasonCode)
          ? await setBlockedCooldown(
              produto.ml_item_id,
              reasonCode,
              updateBeforeReactivate.error ||
                statusBeforeReactivate.error ||
                undefined,
            )
          : undefined;
        detalhe = {
          ...
          erro:
            updateBeforeReactivate.error ||
            statusBeforeReactivate.error ||
            "Falha ao preparar anúncio para reativação",
          status:
            updateBeforeReactivate.data?.status ||
            statusBeforeReactivate.data?.status,
          ...
          attempts: totalAttempts,
          http_status:
            updateBeforeReactivate.status || statusBeforeReactivate.status,
        };
        resultado.erros++;
        ...
        if (
          updateBeforeReactivate.transient ||
          statusBeforeReactivate.transient
        ) {
          resultado.erros_transitorios++;
        } else {
          resultado.erros_nao_recuperaveis++;
        ...
        const verifiedStatus =
          statusCheck.data?.status || reactivate.data?.status;
        ...
        if (reactivate.success && verifiedStatus === "active") {
          ...
          const blockedUntilIso = isBlockedByMlRule(reasonCode)
            ? await setBlockedCooldown(
                produto.ml_item_id,
                reasonCode,
                reactivate.error || statusCheck.error || undefined,
              )
            : undefined;
          detalhe = {
            ...detalhe,
            ...
            erro:
              reactivate.error ||
              statusCheck.error ||
              "Não foi possível reativar anúncio",
      ...
      const updateStock = await atualizarQuantidadeItemML(
        produto.ml_item_id,
        estoque,
      );
      const statusCheck = await obterStatusItemML(produto.ml_item_id);
      const verifiedStatus =
        statusCheck.data?.status || updateStock.data?.status;
      ...
      if (updateStock.success) {
        ...
        const blockedUntilIso = isBlockedByMlRule(reasonCode)
          ? await setBlockedCooldown(
              produto.ml_item_id,
              reasonCode,
              updateStock.error || statusCheck.error || undefined,
            )
          : undefined;
        detalhe = {
          ...detalhe,
          ...
          erro:
            updateStock.error ||
            statusCheck.error ||
            "Falha ao atualizar estoque no ML",
        ...
        if (
          isBlockedByMlRule(updateStock.reason_code || statusCheck.reason_code)
        ) {
          resultado.erros_bloqueio_ml++;
          resultado.bloqueios_ml_regra++;
          console.warn(
...
export async function reconciliarPausasEstoqueZero(
  ...
  const manualBlockSet = await getMlManualBlockSet(produtos);
  const manualBlockedDetalhes: Array<{
    ml_item_id: string;
    sku?: string;
    current_status?: string;
    acao: string;
...
/** Entradas legadas aposentadas: alterações usam simulação e aprovação canônicas. */
export async function setItemQuantityPricing(_itemId:string,_basePrice:number):Promise<QuantityPricingApplyResult>{
  throw new Error('ATACADO_REQUER_AVALIACAO_E_APROVACAO_POR_QUANTIDADE');
}
export async function updateItemPrice(_itemId:string,_price:number):Promise<boolean>{
  throw new Error('PRICING_APPROVAL_REQUIRED: usar /api/pricing/simulate e /api/ml/anuncio/atualizar-preco');
}
