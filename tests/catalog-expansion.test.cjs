...
test('exclusão comprovada encerra apenas o stop exato, sem validar publicação ou liberar claim pendente',()=>{
 const stop={id:'stop1',event_type:'CATALOG_EXPANSION_SAFETY_STOP',produto_id:'p',ml_item_id:'MLB1',created_at:'2026-09-07T01:00:00Z'};
 const payload={batchId:CATALOG_EXPANSION_BATCH,outcome:'EXCLUDED',explicitUserAuthorization:true,observedAt:'2026-09-07T01:01:00Z',deletedReadback:{id:'MLB1',status:'inactive',sub_status:['forbidden','deleted']},reconciliation:{resolvedSafetyStopId:'stop1'}};
 const done={event_type:'CATALOG_EXPANSION_SAFETY_STOP_RESOLVED',produto_id:'p',ml_item_id:'MLB1',created_at:'2026-09-07T01:02:00Z',payload};
 assert.doesNotThrow(()=>assertCatalogExpansionCanAdvance([stop,done]));
 for(const patch of [{batchId:'OTHER'},{outcome:'ACTIVE'},{explicitUserAuthorization:false},{observedAt:stop.created_at},{observedAt:'2026-09-07T02:00:00Z'},{reconciliation:{}},{deletedReadback:{id:'MLB2',status:'inactive',sub_status:['deleted']}},{deletedReadback:{id:'MLB1',status:'active',sub_status:['deleted']}},{deletedReadback:{id:'MLB1',status:'inactive',sub_status:['forbidden']}}]) assert.throws(()=>assertCatalogExpansionCanAdvance([stop,{...done,payload:{...payload,...patch}}]),/SAFETY_STOP/);
 for(const patch of [{produto_id:'q'},{ml_item_id:'MLB2'},{created_at:stop.created_at}]) assert.throws(()=>assertCatalogExpansionCanAdvance([stop,{...done,...patch}]),/SAFETY_STOP/);
 assert.throws(()=>assertCatalogExpansionCanAdvance([stop,done,{...stop,id:'stop2'}]),/SAFETY_STOP/);
 assert.throws(()=>assertCatalogExpansionCanAdvance([stop,done,{event_type:'CREATE_REQUESTED',produto_id:'p'}]),/RECONCILIACAO/);
});
...
test('relatório conta produtos únicos e estado atual sem reescrever manifesto',async()=>{
 const fs=require('fs'),ts=require('typescript'),vm=require('vm');const source=ts.createSourceFile('runner.js',fs.readFileSync('scripts/catalog-expansion-batch-01.cjs','utf8'),ts.ScriptTarget.Latest,true);const code=source.statements.find(n=>ts.isFunctionDeclaration(n)&&n.name?.text==='report').getText(source);
 const events=[{event_type:'CREATED_REMOTE',produto_id:'p',ml_item_id:'old',payload:{}},{event_type:'CREATED_REMOTE',produto_id:'p',ml_item_id:'new',payload:{replacementAuthorizationId:'auth'}},{event_type:'CATALOG_EXPANSION_VALIDATED',produto_id:'p',ml_item_id:'new',payload:{}}];let result;const files=[];
