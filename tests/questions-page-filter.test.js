...
test('página concluída usa somente a API real do Mercado Livre', () => {
  const pageSource = fs.readFileSync(
    path.join(__dirname, '../src/app/(app)/perguntas/page.tsx'),
    'utf8',
  );
  ...
  assert.doesNotMatch(pageSource, /BNT_D06_VISUAL_REVIEW|createQuestionHomologationFixtures|isHomologationFixture/);
});
