...
async function quote(options={}) {
  ...
  const remote=id=>({id,seller_id:123,site_id:'MLB',currency_id:'BRL',price:100,status:options.closed?'closed':'active',
  ...
  Object.assign(mocks,{
    ...
    '@/services/pricing-market-quote':{quoteMoney:v=>Math.round(v*100),observedMarketContext:(item,sellerId)=>String(item.seller_id)!==sellerId?null:({
      sellerId,itemId:item.id,categoryId:item.category_id,catalogProductId:null,listingType:item.listing_type_id,condition:item.condition,
      mode:item.shipping.mode,logisticType:item.shipping.logistic_type,freeShipping:item.shipping.free_shipping,dimensions:null,currency:'BRL',quantity:1})},
...
test('price quote checks live identity/eligibility as warnings without blocking the manual decision',async()=>{
  ...
  for(const option of ['inactive','closed','blocked','originConflict','peerConflict','categoryUnavailable']) {
    const warned=await quote({[option]:true});assert.equal(warned.body.decisionContext.executable,true,option);
    assert.ok(warned.body.decisionContext.warnings.length>0,option);
  }
});
test('commercial identity change does not invalidate the technical price fingerprint',async()=>{
  assert.equal((await quote()).body.decisionContext.fingerprint,(await quote({brand:'B'})).body.decisionContext.fingerprint);
});
